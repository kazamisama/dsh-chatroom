...
import { createChatroomStore, shortId, parseMentions, saysNoReply, detectOverreach, VERDICTS } from './rooms.js'
import { verifyDeclaration, describeVerification, VERIFIED } from './gitcheck.js'
...
function apply(ctx) {
  ...
  async function relatedMemberIds(roomId, declarerId, workspace, files) {
    ...
    if (members.length === 0 || files.length === 0) return out
    const workspaces = await memberWorkspaceMap(roomId)
    const declared = files.map((f) => path.normalize(f))
    const declWs = workspace === '' ? '' : path.resolve(workspace)
    for (const m of members) {
      if (m.sessionId === declarerId) continue // 声明者知道自己改了什么，不必被自己叫醒
      const subs = Array.isArray(m.subscriptions) ? m.subscriptions : []
      if (subs.length > 0 && declared.some((d) => subs.some((s) => path.normalize(String(s)) === d))) {
        out.push(m.sessionId)
        continue
      }
      const mWs = workspaces.get(m.sessionId) || ''
      if (mWs === '') continue
      const resolved = path.resolve(mWs)
      if (declWs !== '' && resolved === declWs) {
        out.push(m.sessionId) // 同一工作区：任何改动都可能踩到它
        continue
      }
      // 跨工作区：只有「同一相对路径的文件确实存在于它的工作区」才算相关，
      // 否则同名文件会把无关成员全部误伤。
      let hit = false
      for (const rel of declared) {
        try {
          await fs.access(path.join(resolved, rel))
          hit = true
          break
        } catch {
          /* 该文件不在它的工作区 */
      ...
      if (hit) out.push(m.sessionId)
    }
    debug('确定性匹配: ' + out.length + '/' + members.length + ' 名成员可能相关')
    return out
  }
   ...
   */
  function directionHint(member) {
    const mine = String(member.selfDescription || '')
    return mine !== ''
      ? ' ｜ 你声明过的方向：「' + mine + '」（若这次动作越出这个范围，请在回执里说明）'
      : ' ｜ 你还没在房间里声明方向 —— 先调 room_intent 说清你负责什么，别人才判断得了边界'
  }
  ...
  disposers.push(tools.register({
    name: 'room_status',
    description: '查看聊天室现状：成员、身份、谁还没表态、最近的变更。不传 room 则列出全部房间。',
    parameters: {
      type: 'object',
      properties: { room: { type: 'string', description: '房间 id 或名称；省略则列出全部房间。' } },
      ...
      for (const room of picked) {
        ...
        for (const m of st.members) {
          lines.push('  - [' + shortId(m.sessionId) + ']'
            ...
            + '  已读 ' + m.lastReadSeq
            + (m.owed ? (m.overdue ? '  ← 欠一次表态（已逾时）' : '  ← 欠一次表态') : '')
            + (m.verdict ? '  已表态: ' + m.verdict : ''))
        }
        if (st.pending.length > 0) lines.push('  待表态: ' + st.pending.map(shortId).join(', '))
      }
  ...
  disposers.push(tools.register({
    ...
    execute: async (args, exec) => {
      ...
      const found = parseMentions(text, store.activeMembers(room.id), sessionId)
      // 但**作者说了不用回，就不用回**（#55 报的）：正文写明「不需要回应」，
      // 或者显式传 wake=false。两种都只投背景，不登记义务 —— 只是"提到"，不是"点名"。
      const byText = saysNoReply(text)
      const byFlag = args.wake === false
      const suppressed = (byText || byFlag) && found.length > 0
      const mentions = suppressed ? [] : found
      ...
      return {
        ...
        text: '已发言 #' + message.seq
          + (suppressed
            ? '（提到 ' + found.length + ' 人，但' + (byFlag ? 'wake=false' : '正文写了「不需要回应」')
              + '：只作背景投递，没有登记义务、没唤醒谁）'
            : (mentions.length > 0
              ? '（@ 了 ' + mentions.length + ' 人，已唤醒 ' + woke + ' 人 —— 他们必须回一句）'
              : '')),
      }
    },
  }))
  ...
  disposers.push(tools.register({
    ...
    parameters: {
      ...
      properties: {
        room: { type: 'string', description: '房间 id 或名称。' },
        files: { type: 'array', items: { type: 'string' }, description: '改动的工作区相对路径列表。' },
    ...
    execute: async (args, exec) => {
      ...
      const workspace = (exec && exec.agent && exec.agent.session && exec.agent.session.header
  ...
  disposers.push(tools.register({
    name: 'room_intent',
    description: '声明你当前负责的**设计方向与约束**（一句话）。房间会把它显示给所有成员 —— '
      + '在问「谁改了文件」之前，先看「谁在负责什么」，这是避免会话间设计偏离最直接的办法。'
      + '方向变了就再调一次覆盖。',
    parameters: {
      type: 'object',
      properties: {
        room: { type: 'string', description: '房间 id 或名称。' },
        direction: { type: 'string', description: '一句话说清你负责的方向（例：只动前端表现层，不碰数据模型与接口）。' },
      },
    ...
    execute: async (args, exec) => {
      ...
      const updated = await store.setSelfDescription(room.id, sessionId, String(args.direction))
      if (updated === null) return { text: '你不是这个房间的成员，无法声明方向（请让用户把你加入房间）。' }
      return { text: '方向已记录，房间里所有人都能看到：「' + updated.selfDescription + '」' }
    },
  }))
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '- 想知道现状用 room_status；想说话用 room_say —— 默认**不**要求别人回应，只是背景上下文。',
        ...
        '- 用 room_intent 声明你当前负责的设计方向与约束。很多"设计偏离"不是谁改了文件造成的，',
  ...
  /** 面板要的一屏数据（异步：成员要补会话标题）。 */
  async function snapshot(roomId) {
    const st = store.status(roomId)
    if (st === null) return null
    const messages = store.state.messages.filter((m) => m.roomId === roomId).slice(-100)
    const titles = await titlesFor(st.members.map((m) => m.sessionId))
    return {
      room: st.room,
      lastSeq: st.lastSeq,
      targetSeq: st.targetSeq,
      members: st.members.map((m) => ({ ...m, title: titles.get(m.sessionId) || '' })),
      pending: st.pending,
      messages,
  ...
  async function correctHistoricalVerdicts() {
    ...
    try {
      ...
      const pending = store.state.changes.filter((c) => c.verdict === 'contradicted').slice(0, 20)
      ...
      for (const change of pending) {
        ...
        // 「从什么改成什么」里的**前者恒为 contradicted** —— 那是上一条筛选条件，不是从对象上读来的值。
        // （早先写成 change.verdict，被并发的那一次改掉之后就成了 "verified → 已证实"。）
        const from = 'contradicted'
  ...
  ctx.inject(['connection'], (connectionCtx) => {
    ...
    const handleRpc = async (endpoint, payload) => {
      ...
      try {
        switch (endpoint) {
          case 'state':
            return ok({ rooms: await Promise.all(store.listRooms().map((room) => snapshot(room.id))) })
          case 'create-room': {
