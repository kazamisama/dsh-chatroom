...
import {
  ...
  memberOwnership, matchesOwnedPath, suspectGlobs, DIRECTION_MAX_CHARS,
} from './rooms.js'
import { verifyDeclaration, describeVerification, resolveWorktree, criteriaFingerprint, VERIFIED } from './gitcheck.js'
...
function apply(ctx) {
   ...
   */
  const REJUDGE_VERSION = 1

  /**
   * 重判的**判定输入装配** —— 三个输入都在这里：锚点（声明者会话的开始时间）、ref（原声明的 commit）、
   * 仓库路由（按**声明文件**回溯，不与"上次核验的答案"挂钩）。
   *
   * 它**本身是判据的一部分**，所以它的源码进了 rejudgeStamp 的指纹（837e0518 在 #3638② 点出来的边界）：
   * 指纹原来只覆盖 lib/gitcheck.js，而真机 #1454/#1458 修的是**这里的路由** —— 那处改动若发生在盖章之后，
   * 它修好的那批记录**永远不会**被重判回来：「自愈静默失效」换个地方再犯一次。
   *
   * @returns 交给 verifyDeclaration 的参数；**锚点拿不到就返回 null**（调用方跳过，且**不盖章**）。
   */
  async function rejudgeInputs(change, starts) {
  ...
  /**
   * 自动挡的另一分量：**重判输入装配函数的源码摘要**。
   * 判据不止在 lib/gitcheck.js 里 —— 锚点/ref/路由是在 index.js 这边装配的，改了它们，章也得跟着变。
   */
  function assemblyFingerprint() {
    try {
      return 'i' + createHash('sha256').update(String(rejudgeInputs)).digest('hex').slice(0, 8)
    } catch {
      return 'i0'
    }
  }
   ...
   *
   *     g[lib/gitcheck.js 的字节摘要] | i[重判输入装配的源码摘要] | v[手工版本号]
   *
   * 前两段是**自动挡**：改任一处判据（gitcheck.js 里的判定，或这边装配的锚点/ref/路由），章自己就变，
   * 不需要谁记得 +1 —— 而这正是 #3638② 那个边界：手工挡原来只在"读不到源码"时才生效，
   * 等于**没有任何办法主动触发一轮重判**，而 index.js 那边的判据改动**没有任何东西会发现**。
   * 第三段是**手工挡**，用途见 REJUDGE_VERSION 的说明（兜底 + 扳机）。
   */
  const rejudgeStamp = () => (criteriaFingerprint() || 'g0') + '|' + assemblyFingerprint() + '|v' + REJUDGE_VERSION

  /** 一批最多重判多少条：一次启动别把 git 跑爆，剩下的留给下一次 tick。 */
  const REJUDGE_BATCH = 20
