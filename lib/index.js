...
function apply(ctx) {
  ...
  function boundaryText(sessionId) {
    ...
    for (const room of store.listRooms()) {
      ...
      const dir = String(m.selfDescription || "").trim()
   ...
   *   watch=all   —— 观察者/审计席：**全推**（变更通知与闲聊都进上下文）
  ...
  function skipInject(member, message) {
    ...
    if (member.watch === 'all') return false     // 观察者席：全推
  ...
  disposers.push(tools.register({
    ...
    execute: async (args, exec) => {
      ...
      const watch = updated.watch === 'all' || updated.watch === 'none' || updated.watch === 'quiet' ? updated.watch : null
      // 收录范围说清了就不该再催它补 paths —— 观察者席位**故意**没有领地
      // （真机 #1714：工具曾对着审计席建议"补 paths 就能收敛"，那条建议对它是错的）。
      const scope = watch === 'all'
        ? '收录范围：**观察者/审计席**（全推：变更通知与闲聊都进上下文）'
        : (watch === 'quiet'
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '  要看用 room_message(seq) / room_status 拉（房间历史一条不少）。审计/观察席要全量就 room_intent 传 watch="all"；',
        '  连变更唤醒也不要则传 watch="none"。命中你 paths 的变更**仍会叫醒你**（那是义务，不是推送）。',
