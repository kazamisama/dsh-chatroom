...
function apply(ctx) {
  ...
  /**
   * 这条**变更通知**要不要干脆不推给这个成员（背景通道的收窄）。
   *
   * 判据只有两条，都是确定性的：
   *   ① 它声明了 watch=none → 不推（静音席位）；
   *   ② 它有**结构化边界**（paths/订阅）且这次改的文件一条都不在其中 → 不推。
   * 其余一律照推：没给边界的人享受不到这条（对它是猜测）；watch=all 是观察者席，全推。
   *
   * 不推 ≠ 看不见：room_message(seq) 与 room_status 仍能拉到全文，
   * 房间的历史与审计链一条不少（变的只是"推"与"拉"）。
   */
  function skipChangeInject(member, message) {
    if (member.watch === 'all') return false
    if (member.watch === 'none') return true
    const own = memberOwnership(member)
    if (!own.structured) return false
    const change = store.state.changes.find((c) => c.roomId === message.roomId && c.seq === message.seq)
    const files = change === undefined || !Array.isArray(change.files) ? [] : change.files
    if (files.length === 0) return false // 拿不到文件清单就不敢省这一步
    return !files.some((f) => matchesOwnedPath(f, own.owned) !== null)
  }
  ...
  async function fanout(roomId, seq) {
    ...
    for (const member of store.activeMembers(roomId)) {
      ...
      if (message.kind === 'change-notice' && skipChangeInject(member, message)) continue
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '- **变更通知按领地推送**（2026-09-16 收窄）：给了 paths 的人只会收到**命中自己边界**的变更全文；',
        '  其余的**不推**（要看你仍然 room_message(seq) 拉得到，房间历史一条不少）。没给 paths 的人照旧全收；',
        '  watch="all" 是观察者席（全收）。**人的发言与 @ 你 的消息一律照推**，不受这条影响。',
