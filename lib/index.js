...
import {
  ...
  memberOwnership, matchesOwnedPath, suspectGlobs, DIRECTION_MAX_CHARS, extraFilesOwnerNote,
} from './rooms.js'
...
function apply(ctx) {
   ...
   */
  function expectPathsNote(roomId, expectPaths, selfSessionId) {
    const files = Array.isArray(expectPaths) ? expectPaths : []
    if (files.length === 0) return ''
    const hits = detectOverreach(files, store.members(roomId), selfSessionId)
    ...
    execute: async (args, exec) => {
      ...
      try {
        if (op === 'create') {
          ...
          return { ok: true, text: '任务已建：' + describeTask(room.id, task) + expectPathsNote(room.id, task.expectPaths, sessionId) }
        }
        if (op === 'claim') {
          const task = await store.claimTask(String(args.taskId), sessionId)
          return { ok: true, text: '已认领：' + describeTask(room.id, task) + expectPathsNote(room.id, task.expectPaths, sessionId) }
        }
        if (op === 'update') {
          const patch = {}
          ...
          return { ok: true, text: '任务已更新：' + describeTask(room.id, task) + expectPathsNote(room.id, task.expectPaths, sessionId) }
        }
        if (op === 'list') {
