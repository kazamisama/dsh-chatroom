...
function apply(ctx) {
  ...
  async function ownershipLookup(roomId, workspace, files) {
    ...
    for (const m of members) {
      ...
      // **只收不答席**（真机 #1798）：两根轴里缺的那个角 —— 全推（看得见）但**不产生义务**（不必每条都应一声）。
    ...
    for (const room of store.listRooms()) {
      ...
      const watch = m.watch === "all" ? "全收（观察者/审计席）"
        : (m.watch === "feed" ? "只收不答（全推变更、不产生义务）"
          : (m.watch === "none" ? "静音（只有 @ 能叫到）" : "默认（只推 @ 你 与人的发言）"))
   ...
   *   watch=all   —— 观察者/审计席：**全推**（变更通知与闲聊都进上下文）+ 每条都唤醒（义务）
   *   watch=feed  —— **只收不答**：全推，但**不产生义务**（真机 #1798 补的那个角）
   *   watch=quiet —— 默认：只推「@ 你」与「人的发言」；变更与闲聊一律走「拉」
   *   watch=none  —— 同 quiet，且**连变更唤醒也不要**（只有 @ 能叫到它）
   *
  ...
  function skipInject(member, message) {
    ...
    if (member.watch === 'all' || member.watch === 'feed') return false
  ...
  disposers.push(tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        watch: {
          type: 'string',
          enum: ['all', 'feed', 'quiet', 'none'],
          description: '收录范围（**与"有没有领地"是两件事**，只读/审计席位用这个，不要编一条假 paths）。'
            + '两根轴：**推不推** × **要不要每条都应一声**：'
            + 'all = 全推 + 每条变更都唤醒（观察者/审计席：逐条独立核验与表态都是它的活）；'
            + 'feed = 全推、但**不产生义务**（只收不答席：要看得见，不必每条都应一声 —— 真机 #1798 那个岗位）；'
            + 'quiet = **默认**，只推「@ 你」与「人的发言」，其余用 room_message 拉；'
            + 'none = 同 quiet，且连变更唤醒也不要（只有 @ 能叫到它）。',
        },
        excludes: {
          type: 'array',
          items: { type: 'string' },
    ...
    execute: async (args, exec) => {
      ...
      const watch = updated.watch === 'all' || updated.watch === 'none' || updated.watch === 'quiet' || updated.watch === 'feed'
        ? updated.watch : null
      ...
      const scope = watch === 'all'
        ...
        : (watch === 'feed'
          ? '收录范围：**只收不答席**（变更通知全推、进背景通道，但**不产生义务** —— 不会被叫醒表态）'
        : (watch === 'quiet'
          ? '收录范围：**只推 @ 你 的与人的发言**（默认档；其余用 room_message 拉）'
          : (watch === 'none'
            ? '收录范围：**静音**（不收变更唤醒，也不推变更/闲聊；被 @ 时仍会叫到）'
          : (paths.length > 0
            ? '机器读的边界 ' + paths.length + ' 条：' + paths.join(' ')
            : '⚠ 你没给 paths —— 唤醒与越界判定只能去猜这段散文，会误报也会漏报（例：'
              + '方向里写「不进 X 依赖图」时，X 可能被当成你的地盘）。'
              + '若你本来就**不认领**任何路径（只读/审计席），改用 watch="all" 或 watch="feed" 明确表达，别编一条假 paths。'))))
      return {
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '  要看得见但**不必每条都应一声** ⇒ watch="feed"（全推进上下文，不产生义务）；连变更唤醒也不要则传 watch="none"。',
        ...
        '  用 watch="all"（收全量变更、不参与越界判定）、watch="feed"（全推但不产生义务）或 watch="none"（不收变更唤醒，被 @ 仍会叫到）。',
        '  很多"设计偏离"不是谁改了文件造成的，',
