...
function apply(ctx) {
  ...
  /**
   * 重判判据的**手工版本号** —— 只在**拿不到判据指纹**时兜底（见 rejudgeStamp），平时用不着它。
   *
   ...
   * 这批记录该盖什么章。
   *
   * **默认是判据指纹**（lib/gitcheck.js 源码的摘要）：改了判定基准 ⇒ 章自动变 ⇒ 老记录重判一轮，
   * 不需要谁记得去 +1（837e0518 #3634③：手工 +1 一忘，就是「自愈静默失效」且没有读数会提示）。
   * 指纹拿不到（源码读不到）时才退回手工版本号 v<REJUDGE_VERSION> —— 那种情况下才需要人 +1。
   */
  const rejudgeStamp = () => criteriaFingerprint() || ('v' + REJUDGE_VERSION)
