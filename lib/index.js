...
/**
 * 逾期提醒的巡检间隔。**不是**投递间隔：真正决定"多久敲一次"的是 REMIND_COOLDOWN_MS（30 分钟）。
 * 巡检只决定"多久看一眼有没有到点的"——20 秒足够便宜（纯内存扫描，不写盘、不发帧）。
 */
const REMIND_TICK_MS = 20000
/**
 * 投递帧的正文预算（用户 2026-09-25 看到 #5169 的帧："这自动发的是不是太长了？"）。
 * 实测那一帧：正文 1249 字，其中**插件自己加的**"（我的方向：…）"占 526 字（42%）—— 我把自己的声明
 * 放大了一倍多。两条判据决定了修法：
 *   · **存储是全文，投递才有预算**：房间记录不能被截（它是账），被截的只能是送进别人上下文的那一份；
 *   · 截断必须给**出口**：帧里写清"全文多少字、用哪个工具读"，否则读者以为自己看到了全部。
 */
const FRAME_BODY_MAX = 800
/**
 * 超预算时**头尾各留多少**（用户 2026-09-25："是不是只有消息本身投递比较好？"）。
 * 只留头是错的：声明正文的**结论在末尾**（"git 校验 ✓ 已证实：…"），截头等于把读者最需要的那句吃掉。
 * 头留摘要/文件、尾留判定，中间（意图、方向回显）让位 —— 要全文用 room_message 拉。
 */
/**
 * **摘要帧**：声明（change-notice）的投递帧只带摘要 + 文件清单 + 判定一词 + 出口 —— 全文留给自己拉。
 * 用户 2026-09-25："这个在投递链能改进吗？不然的话投递 token 数太大了"。
 * 实测（本房间最近 6 小时）：65 条声明 → **206 帧 / 26 万字符 ≈ 17.4 万 tokens**
 * （义务帧 166 + 背景帧 40，平均正文 1035 字）。帧的作用是"叫你来判断/知情"，不是背全文。
 */
const DIGEST_MAX = 240
/**
 * 「看一眼」的帧（wake 席、不要求回执）压成一行时的上限。
 * 实测（审计席 f008c4f2，最近 2 小时）：推送 175 条 / 158k 字符 ≈ 10.5 万 tokens，其中**必须回一句**的只有 6 条
 * —— 96% 是"看一眼"，却按整篇正文发。宿主本来就会把忙碌期到达的帧拼进同一轮（175 帧 → 30 轮），
 * 所以刀口不在"省轮次"，在**省 token 与注意力**；全文一次 room_message 就能拉。
 */
const WAKE_DIGEST_MAX = 160
/**
 * 摘要帧里追加的文件清单**上限**（超过就写「…共 N 个」）。
 * 目的不是省均值（语料里均长只有 52 字），而是**立界** —— 否则一份 200 文件的声明（整仓改名那种）
 * 会让"帧长有上界"变成假命题。审计席 2026-09-25 给的结构性反例：「均值不等于上界」。
 */
const DIGEST_FILES_MAX = 5
/** 声明以外的长正文（人的发言 / 闲聊）：头 + 尾各留多少。只截头会吃掉末尾的结论。 */
const FRAME_HEAD_MAX = 500
const FRAME_TAIL_MAX = 280
// （原 DIRECTION_ECHO_MAX = 200：声明正文回显方向时的截断上限。2026-09-25 正文不再回显方向 ⇒ 常量一并删。）
 ...
 */
function directionShapeHints(direction, opts = {}) {
  const text = String(direction === undefined || direction === null ? '' : direction)
  // **已知成员短号**：房间寻址人的规范形式就是它（8 位十六进制），而 §9 的【纪律】恰好要求写
  // 「改什么之前先 @ 谁」。只按 sha 形状判，会把**每一条按标准写好【纪律】的声明**都判成"含会腐烂的引用"，
  // 而提示给的修法（改成文档小节号）对短号不适用 ⇒ 作者只能删掉 @ 对象或学会忽略提示（cdfc696d #5148）。
  // 判据能用现成花名册就别靠形状猜。
  const roster = new Set((Array.isArray(opts.shortIds) ? opts.shortIds : [])
    .map((s) => String(s === undefined || s === null ? '' : s).toLowerCase())
    .filter((s) => s !== ''))
  const out = []
  const NEED = ['【面】', '【不碰】', '【纪律】']
  ...
  if (late.length > 0) {
    // 印**段首位置**（1 基），而不是只说"缺" —— 否则作者无从自查自己落在 200 的哪一侧
    // （f008c4f2 #5114③：回显只印总字数，不印那三段各自的起点；步长一对就得靠测试钉）。
    out.push('前 200 字里缺 ' + late.map((tag) => {
      const at = text.indexOf(tag)
      return at < 0
        ? tag + '（整条都没出现）'
        : tag + '（从第 ' + (at + 1) + ' 字才开始 —— 段首须落在第 200 字或之前）'
    }).join(' / ') + '（唤醒帧只回显前 200 字 ⇒ 落在后面的段落别人读不到，只读得到故事）')
  }
  if (text.length > 400) out.push('正文 ' + text.length + ' 字（标准 ≤400）')
  const rot = []
  if (/\d{4}-\d{2}-\d{2}/.test(text)) rot.push('日期')
  // sha 形状 + **过滤**：已知成员短号不算（花名册说话，见上）。**不抬高阈值** ——
  // 本房间真的在用 7 位短 sha（ca84035 那种），要求 >=12 会漏掉真的引用。
  const shaHits = []
  const shaRe = /(^|[^0-9a-f])([0-9a-f]{7,40})(?=[^0-9a-f]|$)/g
  let m = shaRe.exec(text)
  while (m !== null) {
    const token = m[2]
    if (!roster.has(token.toLowerCase())) shaHits.push({ token, at: m.index + m[1].length + 1 })
    shaRe.lastIndex = m.index + m[1].length + token.length
    m = shaRe.exec(text)
  }
  if (shaHits.length > 0) rot.push('commit sha')
  if (/#\d{3,}/.test(text)) rot.push('#seq')
  if (/[\w./-]+\.\w+:\d+/.test(text)) rot.push('file:line')
  if (rot.length > 0) {
    // **报出命中的对象与位置**（cdfc696d #5148 的 (c)）：只说"有一处"作者得通读全文才改得动。
    const where = shaHits.slice(0, 3).map((h) => h.token + '@第 ' + h.at + ' 字').join(' ')
    out.push('含会腐烂的引用（' + rot.join(' / ') + '）'
      + (shaHits.length === 0 ? '' : '：' + where + (shaHits.length > 3 ? ' …共 ' + shaHits.length + ' 处' : ''))
      + ' —— 方向要长期有效，引用请改成文档小节号（**成员短号不算**：那是房间身份，见 §9 的【纪律】）')
  }
...
function apply(ctx) {
  ...
  async function ownershipLookup(roomId, workspace, files) {
    ...
    for (const m of members) {
      ...
      if (m.watch === 'all') {
        row.wouldWake = true
        row.reason = '观察者席位（watch=all）：收全量变更；领地按它自己给的 paths 算（给了就照样参与越界判定）'
        rows.push(row)
        continue
      }
      // **唤醒席**（真机 #1920，用户提的需求）：全推 + **变更叫醒** + **不必回**（普通发言只注入，不叫醒）。
   ...
   * 构造一条房间消息的模型可见文本。
   *
   * **正文预算**（FRAME_BODY_MAX）：帧会进别人的上下文，长正文是实打实的成本 ——
   * 但**存储不动**（房间记录是账）。截断一定带出口，别让读者以为看到了全部。
   * extra（"你必须回一句…"那类行动指令）**不进预算**：它是这一帧存在的理由，不能因为它长就被吃掉。
   */
  function frame(room, message, extra = '', digestOnly = false) {
    ...
    const body = String(message.body === undefined || message.body === null ? '' : message.body)
    // 声明走**摘要帧**：正文 = 摘要… + 文件清单 + 判定一词 + 出口，全文用 room_message 拉。
    // 判定/文件清单取自**结构化记录**（change.verdict / change.files），不靠正则从正文里抠。
    // 截到 'git 校验' 之前 = 作者自己写的那半（判据与历史重判同一处：index.js:2811 也是这么切的）。
    if (message.kind === 'change-notice') {
      const cut = body.indexOf('git 校验')
      const head = (cut > 0 ? body.slice(0, cut) : body).trim()
      const change = store.state.changes.find((c) => c.roomId === message.roomId && c.seq === message.seq)
      // 文件清单**只说一次**：正文里作者已经带了 ' [declared files]'（拼正文时无条件加的），
      // 而这里又要追加一份**结构化**（仓库内相对）清单 —— 活机 #5222 实测同一份文件出现了两次、
      // 还是两种写法（'[dsh-chatroom/lib/index.js]' 与 '[lib/index.js]'），看起来像两组不同的文件。
      // 判据：头部已含**第一个仓库内相对路径**就不再追加（前缀被 worktree 吃掉的情形正好命中；
      // 万一头部碰巧含这个后缀，最坏也只是少重复一遍 —— 头部仍然列着一份）。
      const firstFile = change !== undefined && Array.isArray(change.files) && change.files.length > 0
        ? String(change.files[0])
        : ''
      // ⚠ 判据必须拿**实际显示的那一段**比（shownHead），不能拿整个 head：头部会在 DIGEST_MAX 处截断，
      // 用整段 head 判"已经有了"就会把被截掉的文件清单**整个丢掉**（第一版就这么错的，测试当场咬住 hits=0）。
      //
      // 【这条判据的层次声明 —— 房间 #5231（f008c4f2 审、837e0518 收窄）要求写出来，否则"不许混层"
      //  会被读成"两边必须同层"，而这条恰好是那个说法的反例：】
      //   · **主语层 = 投影**（问题是"读者在**实际显示出来的这段**里看得见这条路径吗"）；
      //   · **读取层 = 同一段投影**（shownHead）；
      //   · 结构化清单（change.files，存储层）**只提供要找的 token**，它不是被读的那一层。
      //   ⇒ 存活的规则是「**读哪一层，由问题的主语在哪一层定**」，不是"两边同层"。
      //   ⇒ 它的第二个方向同样要守：**主语在存储/代码的判据，必须读存储与代码** ——
      //     本函数下面那句判定词就是从 change.verdict（存储）读的，不从正文里那句话推；
      //     测试里两个方向各有一条反例网（见 tests/host.mjs §22 与 §23）。
      const shownHead = head.length > DIGEST_MAX ? head.slice(0, DIGEST_MAX) : head
      // **必须立界**（审计席 f008c4f2 结构性反例，2026-09-25）：无条件 join 意味着 200 个文件的声明
      // 帧长随文件数增长 ⇒ 「帧长不随正文增长」是真命题，但「帧长**有上界**」不成立。
      // 而"平均省 2%"是**语料均值**，与上界是两件事（均值不等于上界）。
      // 超过上限时**始终**给出计数：头部里那串会被 240 字截断，读者否则连"一共几个文件"都看不到。
      // 未超上限时才用"头部已含就不重复"那条省字规则。
      const tooMany = change.files.length > DIGEST_FILES_MAX
      const files = firstFile === '' ? ''
        : (tooMany
          ? ' [' + change.files.slice(0, DIGEST_FILES_MAX).join(', ') + ' …共 ' + change.files.length + ' 个]'
          : (shownHead.includes(firstFile) ? '' : ' [' + change.files.join(', ') + ']'))
      const word = change === undefined ? ''
        : (change.verdict === 'verified' ? ' · git ✓ 已证实'
          : (change.verdict === 'contradicted' ? ' · git ✗ 与事实不符' : ' · git ? 未证实'))
      return '[聊天室 ' + room.name + ' #' + message.seq + '] ' + who + ': '
        + (head.length > DIGEST_MAX ? shownHead + '…' : shownHead) + files + word
        + '（全文 ' + body.length + ' 字：room_message(seq=' + message.seq + ')）' + extra
    }
    // 「看一眼」的帧：一行摘要 + 出口。**不适用于义务帧**（他必须按内容判断，不能只看一行）。
    if (digestOnly) {
      const head = body.slice(0, WAKE_DIGEST_MAX)
      return '[聊天室 ' + room.name + ' #' + message.seq + '] ' + who + ': '
        + head + (body.length > head.length ? '…' : '')
        + '（全文 ' + body.length + ' 字：room_message(seq=' + message.seq + ')）' + extra
    }
    const shown = body.length > FRAME_BODY_MAX
      ? body.slice(0, FRAME_HEAD_MAX) + '\n…（正文共 ' + body.length + ' 字，这帧只发头 ' + FRAME_HEAD_MAX
        + ' + 尾 ' + FRAME_TAIL_MAX + ' 字 —— 全文用 room_message(seq=' + message.seq + ') 读）\n'
        + body.slice(body.length - FRAME_TAIL_MAX)
      : body
    return '[聊天室 ' + room.name + ' #' + message.seq + '] ' + who + ': ' + shown + extra
  }
  ...
  function sendOne(agent, mode, msg, room, extra, digestOnly = false) {
    ...
    const userMessage = createUserMessage({
      content: [{ type: 'text', text: frame(room || { name: '?' }, msg, extra, digestOnly) }],
   ...
   */
  async function deliver(sessionId, message, { mode = 'followup', extra = '', digest = false } = {}) {
    ...
    const ok = sendOne(agent, mode, message, message.room || store.getRoom(roomId), extra, digest)
    if (tracked) {
  ...
  // 【删于 2026-09-25】这里原有 directionHint(member, seq)：投递时把**收件人自己的**方向/边界/收录范围
  // 拼在帧尾。删它的理由是**实测出来的重复 + 漂移**，不是"太长"：
  //   · 同样的信息已经在收件人自己的系统提示词里（boundaryText → section 'chatroom:boundary'，order 161），
  //     而且 text() 是**每次组装现算**的；
  //   · 帧尾那份是 fan-out 那一刻的快照 —— 它自己的注释都写着"DSH 可能把它压在 next-turn 队列里几分钟，
  //     送达时可能已经是假话"（真机 #1714）。
  // 同一个事实两处、一处新一处旧，比只有一处更坏（用户看到 #5180 的帧时问："是不是只有消息本身投递比较好？"）。
  // 越界提示不依赖它：那是由 detectOverreach 显式点名负责人的（只写裸短号，不制造提及）。
  ...
  async function fanout(roomId, seq, opts = {}) {
    ...
    for (const member of obligors) {
      ...
      // 行动指令**每条帧都重复一遍**（实测 206 帧 / 6 小时），所以它自己也要省：
      // 四选一的**语义**在 room_judge 的工具描述与协议段里都有，帧里只留取值与"投递会滞后"这一句。
      const options = ' —— ' + why + '：room_judge（' + VERDICTS.join(' / ') + '）'
        // 投递会滞后：对方可能已经从 room_status 看到这条义务、先表了态，这一帧才落地。
        // 那是回声，不是要它再表一次 —— 明说，别让它去猜（D5 表态是终端的）。
        + '（投递可能滞后：若房间里已显示你就这条表过态，忽略本条即可）'
        // 这里原本还拼一句 directionHint(member, …)（收件人自己的方向/边界/收录范围快照）。
        // 2026-09-25 用户问"是不是只有消息本身投递比较好？"时核出来的事实：那份信息**已经**在收件人自己的
        // 系统提示词里（boundaryText → chatroom:boundary，order 161，text() **每次组装现算**），
        // 而帧尾那份是发帧那一刻的**快照**（本文件上面自己都写着"送达时可能已经是假话"）。
        // 于是它不是"长"，是**重复 + 会漂移**：同一个事实两处，一处新一处旧。删掉帧里的那份。
    ...
    const wakeNote = ' —— 这条只叫你看一眼：**不要求回执**（不回也不会有人等你）'
    for (const member of store.activeMembers(roomId)) {
      ...
      const ok = await deliver(member.sessionId, { ...message, room }, {
        // digest: 这一帧**不要求回执**（看一看就走）⇒ 压成一行，全文靠拉。
        mode: 'followup', extra: wakeNote, digest: true,
      })
    ...
    for (const member of store.activeMembers(roomId)) {
      ...
      // **非义务的帧一律给一行**：背景注入（inject）与"看一眼"（digest 的 followup）是同一种东西 ——
      // 看得到发生了什么 + 主题，要全文一次 room_message 就拉到。义务帧不受影响（他要按内容判断）。
      // 规则一句话：**义务帧给全文（声明给摘要+判定，其余给头+尾），非义务帧给一行。**
      await deliver(member.sessionId, { ...message, room }, { mode: 'inject', digest: true })
    }
    return results
  }
    ...
    execute: async (args, exec) => {
      ...
      for (const room of picked) {
        ...
        for (const m of st.members) {
          ...
          // **提醒的单位是人，不是房间靶子**（cdfc696d #5017② 报的代价；我裁该做，2026-09-25 实现）。
          //
          // 为什么原来的靶子口径站不住（实测，不是推理）：**没有任何路径会重新唤醒一条"已投递但没回"的义务** ——
          // 补投只补**投递失败**的帧（deliveredAt === null），而 overdue 只被报告、没有定时器消费它。
          // 更糟的是方向还反了：补投取的是 pendingDeliveries 里**最老**的那条 ⇒ 旧的欠账反而更容易被重送。
          // 于是旧文案「靶子=会被重新唤醒 / 旧账=不会再被唤醒」在投递层根本不存在，
          // 而它让"一批 9 条定向 @ 只留住 1 条提醒"（真机 #4974–#4982）。
          // 现在逐人报**他自己最老的那条**（其余按更晚列出），而"不会自动重发"这件事只说一次（见下方表尾）。
          let owedText = ''
          if (own.length > 0) {
            const later = own.slice(1)
            owedText = '  ← ' + (own.length === 1
              ? '欠一次表态 #' + own[0]
              : '欠 ' + own.length + ' 条表态，最老 #' + own[0])
              + (m.overdue ? '（已逾时）' : '')
              // "追"的可见性：没有这一行，机器人就是在暗处敲人（追了几次是事实，得能读到）。
              + (m.reminded > 0 ? '（已自动提醒 ' + m.reminded + ' 次）' : '')
              // 送不出去的次数：这是"我没法把提醒交到他手上"的证据，必须和"提醒了几次"一样明说。
              + (m.remindFails > 0 ? '（⚠ 有 ' + m.remindFails + ' 次送不出去）' : '')
              + (later.length > 0 ? '（另有更晚未回: ' + listSeqs(later) + '）' : '')
          }
          ...
          if (m.watch === 'all') {
            boundText = '  · 观察者席位（收全量变更；领地按它给的 paths 算）'
        ...
        if (st.pending.length > 0) {
          // 表尾这一句是**能力说明**，不是逐条判词。它必须跟着实现走：
          // 2026-09-25 用户裁定"追"之前，这里写的是"**不会自动重发提醒**"—— 那句话在当时是真的，
          // 加了逾期提醒之后就成了假话（一份读起来像事实的旧断言）。
          lines.push('  待表态: ' + st.pending.map(shortId).join(', ')
            + '（逐人一条最老的在上；**逾时的会由插件自动提醒**：冷却 30 分钟、最多 3 次，'
            + '声明 feed/none 的席位不追；要立刻催谁另用 room_alert）')
        }
      }
  ...
  disposers.push(registerRoomTool({
      ...
      properties: {
        ...
        retracts: {
          ...
          description: '可选：**撤回你自己**之前某几条消息产生的义务（例：[1601]）。'
            ...
            + '只认作者本人，销掉之后那几条从对方的欠账清单里一并消失（room_status 与面板的「待表态」；真机 #2131③）。',
        },
      },
      required: ['room', 'text'],
      additionalProperties: false,
    ...
    execute: async (args, exec) => {
      ...
      const room = resolveRoom(args.room)
      if (room === null) return { seq: 0, text: '找不到房间：' + args.room }
      // **缺参数不许变成字面量 "undefined"**（真机 2026-09-25 #5149：一条 9 字符的 free 消息，正文逐字就是
      // "undefined" —— 调用方的 text 缺失，String(undefined) 把它变成了字符串发出去，看起来还像条正常消息）。
      // 空正文没有任何信息量，拒掉比发出去便宜。
      const rawText = args.text === undefined || args.text === null ? '' : String(args.text)
      if (rawText.trim() === '') {
        return { seq: 0, text: '正文是空的 —— **什么都没发出去**。room_say 需要非空的 text（缺参数会被 String() 变成字面量 "undefined"，真机 #5149 就这么发出过一条）。' }
      }
      const text = rawText
      ...
      // P5：这条把**房间靶子**推走了 —— 谁原来欠着它、而这条又不让他欠（于是"新 @ 不销旧账"）。
      // 判据取自 store.status（与 room_status / 面板同一份）。措辞按 2026-09-25 后的口径：
      // **提醒的单位是人**（每人最老那条），而"不会自动重发提醒"是**全局事实**，不是这一条造成的
      //（旧文案把"你被推成旧账了"写成"这之后你就没提醒了"，其实之前也不会有）。
      const after = store.status(room.id)
      const stillOwe = after.members.filter((m) => {
        const own = Array.isArray(m.owedSeqs) ? m.owedSeqs : []
        return targetBefore > 0 && own.includes(targetBefore)
      })
      const displaceNote = stillOwe.length === 0 ? ''
        : '；⚠ 这条把房间靶子推到了 #' + after.targetSeq + '：' + stillOwe.length + ' 人（'
          + stillOwe.map((m) => shortId(m.sessionId)).join(' ') + '）**仍欠 #' + targetBefore
          + '**（新 @ 不销旧账）—— 要他们现在就回 #' + targetBefore + '，得用 room_alert 或再 @ 一条'
      return {
        seq: message.seq,
  ...
  disposers.push(registerRoomTool({
    ...
    description: '声明你改动了哪些文件。房间会立刻用 git 事实核验这份声明（已证实 / 未证实 / 与事实不符），'
      ...
      + ' **回执档位（notify）**：默认 routine —— **不登记任何回执义务**，只按各人的收录范围投递'
      + '（wake/feed/all 席会收到这一条，**quiet 席位靠 room_status / room_message 拉** —— 对它"背景投递"并不成立）；'
      + '只有 contract（端到端形状 / 字段 / 枚举变了，谁是读者谁要跟）与 irreversible（删数据 / 改 schema / 进历史）'
      + '才让相关成员**必须回一句**。**越界**（改了别人声明负责的文件）**任何时候都登记义务**，与档位无关。'
      + // ⚠ 原来这里写"正文里的 @ 任何时候都登记义务"，会被读成**声明正文**里的 @ 也算 ——
        // 而声明通道**根本不解析 @**（只有 room_say 那条通道解析；tests/host.mjs §18 有反向断言）。
        '（**@ 提及只由 room_say 解析**：声明正文里的 @ 不登记任何义务。）',
    parameters: {
      type: 'object',
      properties: {
        room: { type: 'string', description: '房间 id 或名称。' },
        ...
        notify: {
          type: 'string',
          enum: ['routine', 'contract', 'irreversible'],
          description: '回执档位（默认 routine）。routine：只作背景投递，**不登记任何回执义务**；'
            + 'contract：端到端形状 / 字段 / 枚举变了 —— 相关成员必须回一句；'
            + 'irreversible：删数据 / 改 schema / 进历史 —— 相关成员必须回一句，消息里带〔不可逆〕标记。'
            + '越界点名与正文里的 @ 不受档位影响（它们任何时候都登记义务）。',
        },
    ...
    execute: async (args, exec) => {
      ...
      // **fail fast**：summary 缺失比空正文更糟 —— 它照样会被 git 核验、盖章，然后一条 summary 是字面量
      // "undefined" 的声明留在房间里（真机 2026-09-25 #5149 就是这么出去一条 free 消息的，同一个 String() 陷阱）。
      // 所以它在**任何核验之前**就拒掉：不跑 git、不算越界、不落任何记录。
      const rawSummary = args.summary === undefined || args.summary === null ? '' : String(args.summary)
      if (rawSummary.trim() === '') {
        return { seq: 0, verdict: 'unknown', text: 'summary 是空的 —— **没有登记任何声明**。它会被 git 核验并盖章，所以不能是空串或字面量 "undefined"。' }
      }
      // **回执档位**（用户 2026-09-25 裁定"改"）：routine 是默认 —— **不登记任何回执义务**，
      // 相关成员照样看得到（走背景通道 inject），但没人必须回。只有 contract / irreversible 才把"相关"变成义务。
      // 用户当时的两条附带判据：②③ 由**作者显式声明**（不按路径形状猜，今晚的账证明猜形状一路咬人）；
      // 越界与正文 @ 不受档位影响 —— 那两条是"不可能悄悄发生"的保证，不能被人自己调低。
      const TIERS = ['routine', 'contract', 'irreversible']
      const tier = TIERS.includes(args.notify) ? args.notify : 'routine'
      const TIER_MARK = { contract: '〔契约〕', irreversible: '〔不可逆〕' }
      ...
      const body = rawSummary
        + (tier === 'routine' ? '' : ' ' + TIER_MARK[tier] + '（这一档要求相关成员各回一句）')
        + (files.length > 0 ? ' [' + files.join(', ') + ']' : '')
        + (args.intent ? ' — ' + args.intent : '')
        // 【2026-09-25 删】原先这里回显"（我的方向：…）"。删它的两条理由，都是实测出来的：
        //   · **挤占**：摘要帧的头部只有 240 字，而活机 #5222 实测「（我的方向：」从第 134 字开始 ⇒
        //     头部里 45% 被它吃掉 —— 留它的代价不是"多 200 字"，是把摘要挤出帧；
        //   · **会过期**：方向是可变的（room_intent 随时覆盖），而正文是**不可变的日志记录** ⇒
        //     作者一改方向，所有旧声明里那份就成了过期数据，且读者没法分辨。
        // 要看方向有三处**现算**的：面板成员行（给人）、room_status、room_owners(paths)。
        // 只留**一个事实**：这个作者有没有声明过方向（它不随方向变化，读者据此知道这条声明的边界不可靠）。
        + (mineDir === '' ? ' （⚠ 我还没声明方向）' : '')
        + overreachNote
        + '  ' + mark
      ...
      // 定向义务 = 被越界点名的负责人 ∪（**非 routine 档**才有的）会被叫醒的相关成员里**需要回**的那些
      // （watch=wake 的人照样被叫醒，但不进义务集、不进 mentions：真机 #1920）
      // routine 档下 related 全走背景通道：相关成员下一轮看得到，但没有任何回执义务。
      // watch=all 的席位**自己声明了「每条都要表态」**（全推 + 叫醒 + 必回，BLUEPRINT §9）——
      // 那是它对**自己**的要求。routine 降的是"由路由推断出来的义务"，不能替别人降他自己的档：
      // 否则 watch=all 会变成"收全量、但不必回"，一个没人要的档位（2026-09-25 加档位时被现有测试照出来）。
      const allSeats = new Set(
        store.activeMembers(room.id).filter((m) => m.watch === 'all').map((m) => m.sessionId),
      )
      const obliged = [...new Set([
        ...(tier === 'routine' ? [] : related.filter((id) => !noReplySet.has(id))),
        ...(tier === 'routine' ? related.filter((id) => allSeats.has(id)) : []),
      ...
      store.state.changes.push({
        ...
        declaredFiles: files,
        // **档位要结构化落账**（2026-09-25 量效果时照出来的洞）：只把 〔契约〕/〔不可逆〕写进**正文**里 ⇒
        // 既不可测（我按正文 grep 时，把"声明里描述过这两个词"的也算上了，实测 #5169 就被误判）也不可审。
        // 判据：能被统计的东西不要只活在散文里。
        tier,
  ...
  disposers.push(registerRoomTool({
    ...
    execute: async (args, exec) => {
      ...
      const message = await store.appendMessage({
        ...
        sender: { sessionId, roleName: roleNameOf(room.id, sessionId) },
        kind: 'alert',
        body: args.text === undefined || args.text === null ? '' : String(args.text),
      })
  ...
  disposers.push(registerRoomTool({
      ...
      properties: {
        ...
        watch: {
          ...
          description: '收录范围（**与"有没有领地"是两件事**，只读/审计席位用这个，不要编一条假 paths）。'
            ...
            // ⚠ 2026-09-25 用户实测照出来的措辞错误：原来写"全推 + 叫醒"，读起来像**所有**消息都叫醒。
        // 实现是：wakeOnly 只在**声明**那条路径上算（声明 fanout 传 wakeOnly），普通发言走背景注入 ⇒ **不叫醒**。
        // 所以 wake 与 feed 的差别只在**一根轴**上：变更叫不叫醒。
        + 'wake = 全推 + **变更叫醒**，但**不必回**（唤醒席：声明/变更醒过来看一眼，看完不必写话；'
        + '**普通发言只注入、不叫醒** —— 与 feed 的差别就在"变更叫不叫醒"这一根轴上。真机 #1920）；'
    ...
    execute: async (args, exec) => {
      ...
      // 花名册喂给提示器：成员短号（8 位十六进制）**不是**会腐烂的引用 —— 它正是【纪律】要求写的东西。
      const shapeHints = directionShapeHints(updated.selfDescription, {
        shortIds: store.activeMembers(room.id).map((m) => shortId(m.sessionId)),
      })
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '  要**被叫醒但不必每条都应一声** ⇒ watch="wake"（全推进上下文；**变更/声明会叫醒你**，普通发言只注入、不叫醒；不登记义务）；',
        ...
        '  用 watch="all"（收全量变更 + 每条必回）、watch="wake"（全推 + **变更叫醒**、不必回）、watch="feed"（全推、不叫醒）或 watch="none"（不收变更推送；普通发言里 @ 仍会叫到）。',
        '  很多"设计偏离"不是谁改了文件造成的，',
        ...
        // 【2026-09-25 改】原来这里写的是"消息末尾会附上你自己声明过的方向" —— 帧尾那份**已经删掉**了
        // （它与这条线上面那段「你的边界」重复，且是发帧那一刻的快照、会漂移）。一句真话变成假话必须跟着改，
        // 否则读者会去找一个不存在的字段，甚至以为自己漏看了。
        '- 你自己的方向与边界在**你自己的系统提示词**里（每次组装现算）；消息帧**不再重复**它。',
        '- 要判「这块归谁」用 room_owners(paths) 或 room_status（都读当前值），不要凭印象。',
  ...
  // 逾期提醒（"追"；用户 2026-09-25 裁定"追"）。
  // 在此之前**没有任何路径会重发**一条已投递但没回的义务：欠账只被读出来，不会自己找上门。
  // 边界（写在代码里，因为它们是这套东西最容易被做坏的地方）：
  //   · 只追**逾期**的（阈值与面板上那个"逾时"同一个：DEFAULT_OVERDUE_MINUTES）；
  //   · 同一条义务冷却 30 分钟、最多 3 次 —— 追到第 4 次就不再敲，只让它在 room_status/面板里可见；
  //   · 明确声明 watch=feed（看得见但别叫醒我）或 none 的席位**不追** —— 那是它们声明的合约；
  //   · 已表过态的不追（openObligations 那层已经排除，这里不再重复判断）。
  ctx.effect(() => {
    // `at` 只给测试用（"假装现在几点"）：逾期是个**时间陈述**，而测试不该靠等真实时间来证明它。
    // 生产路径永远不传 —— 定时器与 apply 时的首跑都是无参调用。
    const pass = async (at) => {
      const targets = store.reminderTargets(at)
      if (targets.length === 0) return []
      const sent = []
      for (const t of targets) {
        const room = store.getRoom(t.roomId)
        const message = store.state.messages.find((m) => m.roomId === t.roomId && m.seq === t.seq)
        if (room === null || message === undefined) continue
        const extra = '⏰ **逾期提醒**：你在房间「' + room.name + '」还欠 #' + t.seq + ' 的表态'
          + '（已逾时 ' + Math.round(t.ageMs / 60000) + ' 分钟'
          + (t.count > 0 ? '，这是第 ' + (t.count + 1) + ' 次自动提醒' : '') + '）。'
          // 与义务帧同一口径（真机实测发现的活机不一致：义务帧已压短，提醒帧还是长格式）。
          + '用 room_judge 回一句即可销账（' + VERDICTS.join(' / ') + '）'
          // 与义务帧同一条纪律：投递会滞后，别让人以为要再表一次。
          + '（若房间里已显示你就这条表过态，忽略本条即可）'
        const ok = await deliver(t.sessionId, { ...message, room }, { mode: 'followup', extra })
        // **先发后记**：送达失败也记一次？不 —— 失败**不记**，下一轮（20 秒后）再试。
        // 记了就等于把一次没送到的提醒算成"敲过了"，而对方根本没被打扰到。
        if (ok) store.markReminded(t.roomId, t.sessionId, t.seq, at)
        // 失败**也要留下可见的痕迹**（不记账 ≠ 不记录）：否则"对方长期收不到"在房间与面板上都不出现。
        else store.noteRemindFailure(t.roomId, t.sessionId, t.seq, at)
        // roomId 必须带上：调用方（含测试）要按房间收窄，而"在哪间房欠的"本来就是这条事实的一部分。
        sent.push({ roomId: t.roomId, sessionId: t.sessionId, seq: t.seq, delivered: ok, count: t.count + 1 })
      }
      if (sent.some((s) => s.delivered)) await store.persist()
      if (sent.length > 0) {
        debug('逾期提醒：' + sent.map((s) => shortId(s.sessionId) + '#' + s.seq
          + (s.delivered ? '(第' + s.count + '次)' : '(投递失败)')).join(' '))
      }
      return sent
    }
    const timer = setInterval(() => { void pass() }, REMIND_TICK_MS)
    const dispose = () => clearInterval(timer)
    // 测试的钩子：定时器不该决定"这条能不能被测"。测试**不该**等 20 秒，也不该依赖真实时间。
    dispose.tick = pass
    void pass()
    return dispose
  }, 'chatroom: 逾期提醒')
