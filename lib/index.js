...
/** 声明正文里回显"我的方向"的上限。200 不是随便取的：四段标准保证【面】【不碰】【纪律】落在前 200 字内。 */
const DIRECTION_ECHO_MAX = 200
  ...
  disposers.push(registerRoomTool({
    ...
    execute: async (args, exec) => {
      ...
      const body = rawSummary
        ...
        // 声明者的方向**不能原样塞进正文**：它会长在每一条声明上（真机 #5169 实测 526 字 / 占正文 42%），
        // 而方向本来就存在成员记录里（room_status 随时可读）。留前 200 字足够判"这算不算越界"——
        // 四段标准保证【面】【不碰】【纪律】就在前 200 字内，所以这不是有损的。
        + (mineDir === '' ? ' （⚠ 我还没声明方向）'
          : ' （我的方向：' + (mineDir.length > DIRECTION_ECHO_MAX
            ? mineDir.slice(0, DIRECTION_ECHO_MAX) + '…（共 ' + mineDir.length + ' 字，全文 room_status）'
            : mineDir) + '）')
        + overreachNote
        + '  ' + mark
