...
import {
  createChatroomStore, shortId, parseMentions, saysNoReply, detectOverreach, VERDICTS,
...
function apply(ctx) {
  ...
  function directionHint(member) {
    ...
    const bound = paths.length > 0
      ...
      : ''
    if (mine === '' && bound === '') {
      return ' ｜ 你还没在房间里声明方向 —— 先调 room_intent 说清你负责什么，别人才判断得了边界'
    }
    ...
    return ' ｜ ' + text + bound + '（若这次动作越出这个范围，请在回执里说明）'
  }
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: () => [
        ...
        '  唤醒谁、算不算越界只看 paths（不给就退回猜散文，会误报也会漏报）。很多"设计偏离"不是谁改了文件造成的，',
