...
function apply(ctx) {
   ...
   * 重判判据的**版本号：改了判定基准就 +1**。
   *
   * 为什么需要它（真机 2026-09-20 自查出来的）：重判队列原来是「所有 contradicted/unverified 的
   * **前 20 条**」，而**判对了的红**永远留在队列里 ⇒ 队列只涨不落：超过 20 条之后，一条**新的假红**
   * 排在末尾就再也轮不到重判 —— 自愈**静默失效**（当天实测队列 12 条，只剩 8 格）。
   * 按版本盖章之后：每个版本、每条记录只重判一次，后来的不会被前面的堵住。
   */
  ...
  async function correctHistoricalVerdicts() {
    ...
    try {
      ...
      // **按判据版本过滤**，不是"前 20 条"：判对了的红永远留在队列里，取前 20 条会让它只涨不落，
      // 排在后面的新假红永远轮不到（见 REJUDGE_VERSION 的说明）。
      const pending = store.state.changes
        .filter((c) => c.verdict === 'contradicted' || c.verdict === 'unverified')
        .filter((c) => c.rejudgedUnder !== REJUDGE_VERSION)
      ...
      for (const change of pending) {
        ...
        // 拿不到锚点就不动它（宁可少判），**也不盖章** —— 会话表可能是还没挂载完，
        // 盖了章就等于把"这次没查到"当成"它没问题"，下次再也不会试。也不占这一批的额度。
        if (typeof anchorMs !== 'number' || anchorMs <= 0) continue
        examined++
        ...
        // 翻没翻都盖上这个版本的章：判据没变就不必把同样的 git 再跑一遍
        // （不盖章的话，一次启动要把这些记录全验一遍，下次启动再来一遍 —— 而这套东西的判据很少变）
        change.rejudgedUnder = REJUDGE_VERSION
