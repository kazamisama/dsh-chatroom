...
export function createChatroomStore({ root, now = () => Date.now(), newId = () => Math.random().toString(36).slice(2, 10) }) {
  ...
  async function setSelfDescription(roomId, sessionId, text, { paths = null, excludes = null, watch = null } = {}) {
    ...
    if (watch !== null && watch !== undefined && watch !== 'all' && watch !== 'none' && watch !== 'quiet' && watch !== 'feed') {
      throw new Error('watch 只能是 "all"（全推 + 每条变更都唤醒）/ "feed"（全推、但**不产生义务**）/ '
        + '"quiet"（默认：只推 @ 你 与人的发言）/ "none"（连变更唤醒也不要），收到 '
    ...
    //   feed  = 全推，但**不产生义务**（只收不答席：看得见，不必每条都应一声，真机 #1798）
    //   quiet = **默认**：只推「@ 你」与「人的发言」，其余走「拉」
    //   none  = 同 quiet，且连变更唤醒也不要
    // 两根轴本来各有两档（推/不推 × 有义务/无义务），旧模型只落了三个角 —— feed 就是缺的那个角。
  ...
  function status(roomId, seq = null, overdueMinutes = DEFAULT_OVERDUE_MINUTES) {
    ...
    return {
      ...
      members: members(roomId).map((m) => ({
        ...
        // 'all' | 'feed' | 'quiet' | 'none' | null —— 收录范围那一根轴（与"有没有领地"正交，见 setSelfDescription）
        // null 就是"没声明"，行为上等同 quiet（2026-09-16 用户定的默认）。
        watch: m.watch === 'all' || m.watch === 'none' || m.watch === 'quiet' || m.watch === 'feed' ? m.watch : null,
