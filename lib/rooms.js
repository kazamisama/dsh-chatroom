...
import { assertMessageAppend, assertJudgment, assertTaskWrite } from './invariants.js'
 ...
 */
export function extraFilesOwnerNote(extraFiles, members, selfSessionId) {
  const files = Array.isArray(extraFiles) ? extraFiles : []
  if (files.length === 0) return ''
  const hits = detectOverreach(files, members, selfSessionId)
