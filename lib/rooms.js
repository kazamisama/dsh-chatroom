...
export function createChatroomStore({ root, now = () => Date.now(), newId = () => Math.random().toString(36).slice(2, 10) }) {
  ...
  async function setSelfDescription(roomId, sessionId, text, { paths = null, excludes = null, watch = null } = {}) {
    ...
    if (watch !== null && watch !== undefined && watch !== 'all' && watch !== 'none') {
      throw new Error('watch 只能是 "all"（收全量变更）或 "none"（不收变更唤醒），收到 ' + JSON.stringify(watch))
    }
    ...
    // **观察者/静音席位**（真机 #1714）：收录范围与"有没有领地"是**两个问题**，旧模型把它们当成一根轴，
    // 于是只读席位只能二选一：编一条假领地（= 假所有权），或吃下全量唤醒（= 噪声）。
    // watch 就是那第二根轴：all = 收全量变更但不参与越界判定；none = 不收变更唤醒（@ 仍叫得到）。
  ...
  function status(roomId, seq = null, overdueMinutes = DEFAULT_OVERDUE_MINUTES) {
    ...
    return {
      ...
      members: members(roomId).map((m) => ({
        ...
        // 'all' | 'none' | undefined —— 收录范围那一根轴（与"有没有领地"正交，见 setSelfDescription）
        watch: m.watch === 'all' || m.watch === 'none' ? m.watch : null,
