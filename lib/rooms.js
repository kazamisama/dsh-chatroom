 ...
 *
 * 为什么（真机 #1970，同族）：审计席在自己的量具里发现 `@[0-9a-f]{8}` 会把 `@1789318461`
 * （一个 10 位串）读成 `@17893184` —— 「更长 id 的前缀命中」。**同一个前缀命中我的解析器里也有**
 * （旧写法是 `live.indexOf('@' + short) >= 0`），而这条通道的后果更重：它会**唤醒一个成员**、
 * 登记一条「你必须回一句」的义务。所以这里要求 token 后面不能紧跟 `[0-9A-Za-z_-]`。
 * 角色名里的中文不受影响 —— 前缀匹配角色名是想要的宽松度（`@审计` 命中「审计员」）。
 */
function mentionsToken(text, token) {
  if (token === '') return false
  const esc = token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  return new RegExp('@' + esc + '(?![0-9A-Za-z_-])').test(text)
}
...
export function parseMentions(text, members, senderId = null) {
  ...
  for (const member of list) {
    ...
    else if (role !== '' && mentionsToken(live, role)) ids.push(member.sessionId)
  }
  return ids
}
