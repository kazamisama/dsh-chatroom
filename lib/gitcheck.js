...
export async function resolveWorktree({ workspace, files }) {
  ...
  if (await isWorktree(base)) {
    return { workspace: base, files: declared, fallback: false, reason: 'session-workspace-is-worktree' }
  }

  // 同一趟回溯里，同一个目录只问 git 一次（多个文件时避免重复爬同一串祖先）。
  const tried = new Map()
  const isRepo = async (dir) => {
    if (!tried.has(dir)) tried.set(dir, await isWorktree(dir))
    return tried.get(dir)
  }

  const abs = (f) => path.resolve(base, toRelative(base, f))
  for (const file of declared) {
    let dir = path.dirname(abs(file))
    for (;;) {
      if (await isRepo(dir)) {
        const inside = declared.map((f) => path.relative(dir, abs(f)))
        if (inside.every((rel) => rel !== '' && !rel.startsWith('..') && !path.isAbsolute(rel))) {
          return {
            workspace: dir,
            files: inside.map((rel) => rel.replace(/\\/g, '/')),
            fallback: true,
            reason: 'session-workspace-not-a-worktree',
          }
        }
        break // 这个仓库装不下全部声明文件 —— 换下一个声明文件再试
      }
  ...
  return { workspace: base, files: declared, fallback: false, reason: 'no-worktree-found' }
}
...
export async function verifyDeclaration({
  ...
  if (typeof ref === 'string' && ref.trim() !== '') {
    const kind = await git(workspace, ['cat-file', '-t', ref.trim()])
    if (kind !== null && kind.trim() === 'commit') {
      const ancestor = await git(workspace, ['merge-base', '--is-ancestor', ref.trim(), 'HEAD'])
      if (ancestor !== null) {
        const touched = await git(workspace, ['show', '--name-only', '--format=', ref.trim()])
        if (touched !== null) {
  ...
  return { verdict, reason, files: perFile, head, diffStat, comparedTo, anchorMs: anchor, anchorLabel }
}
...
export function describeVerification(result) {
  ...
  const repoNote = result.repoFallback === true && typeof result.workspaceUsed === 'string' && result.workspaceUsed !== ''
    ? '（git 事实取自 ' + path.basename(result.workspaceUsed) + ' 仓库 —— 会话工作区不在 git 里，按声明文件定位）'
    : ''
  if (result.verdict === VERIFIED) {
    const how = result.files
      .map((f) => f.path
        + (f.dirty ? '（有未提交改动）' : '')
      ...
      .join('、')
    return 'git 校验 ✓ 已证实：' + how + repoNote
  }
  ...
  return 'git 校验 ? 未证实（' + why + '）—— 查不到不等于撒谎，自行判断' + repoNote
}
