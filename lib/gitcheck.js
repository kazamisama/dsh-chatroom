...
async function subrepoCandidates(base, declared) {
  ...
  const out = { base, missing: [], candidates: [], subrepos: [] }
  let entries = []
...
function nearMissNote(result) {
  ...
  const head = '（路径提示：按会话工作目录 ' + base + ' 解析，' + missing.join('、') + ' 不存在'
  if (cands.length > 0) {
    const where = cands.slice(0, 3).map((c) => c.dir + '/' + missing[0]).join('、')
    return head + '；但它在 ' + where + ' 里有同名文件 —— 声明的路径是**会话工作目录相对**的，'
      + '要带上那一级（例：' + cands[0].dir + '/' + missing[0] + '）。这只是提示，核验没有替你改路径。）'
  }
  if (repos.length > 0) {
    return head + '；这个目录下面的仓库有：' + repos.slice(0, 5).join('、')
      + ' —— 若改动在其中一个里，声明要带上那一级。这只是提示，核验没有替你改路径。）'
  }
  return head + '。）'
}
