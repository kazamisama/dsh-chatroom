...
/**
 * **判据指纹**：本模块（= 这套判定基准）源码的摘要。
 *
 * 为什么存在（837e0518 在 #3634③ 提的残留风险，2026-09-20）：
 * 历史重判按「这条记录**在本判据版本下**重判过没有」过滤，而"版本"最初是个**手工 +1** 的常量。
 * 手工的东西一定会忘 —— 忘了 +1，老记录带着旧章**再也不会**被重判，于是「被冤枉的标能自己长回来」
 * 这条性质**静默失效**，而且**没有任何读数会提示你**（不是假想：7d3dc86 改的就是判定基准，
 * 它要是发生在盖章之后而没人 +1，那批假红就永远留在原判）。
 *
 * 所以章不写死：**章 = 判据源码自己的摘要**。改了判据 ⇒ 指纹自动变 ⇒ 记录重判一轮
 * （每版本每条一次，代价上限是"一批 20 条"，见 index.js 的 REJUDGE_BATCH）。
 * 调用方拿到 null（源码读不到）时退化成手工版本号 —— 那时才需要人去 +1。
 *
 * @param file 待摘要的文件；默认本模块自己。
 * @returns 'g' + sha256 前 12 位；读不到文件返回 null。
 */
export function criteriaFingerprint(file = fileURLToPath(new URL('./gitcheck.js', import.meta.url))) {
  try {
    return 'g' + createHash('sha256').update(readFileSync(file)).digest('hex').slice(0, 12)
  } catch {
    return null
  }
}
...
async function subrepoCandidates(base, declared) {
  ...
  // 另一种真机错法：**声明写成父目录相对**（少写一级 ..\，真机 2026-09-20 #3598/#3599）。
  // 那对应的是**兄弟仓**，所以再看一眼上一级目录里、声明点名的那些目录。
  // 同样只用来写提示：能不能采用由 resolveWorktree 的规则决定（那三条都是可核对的）。
  out.siblings = []
  const up = path.dirname(path.normalize(base))
  if (up !== path.normalize(base)) {
    let ups = []
    try { ups = await fs.readdir(up, { withFileTypes: true }) } catch { ups = [] }
    for (const e of ups) {
      if (out.siblings.length >= 5) break
      if (e.isDirectory() !== true) continue
      if (e.name.startsWith('.') || e.name === 'node_modules') continue
      if (path.normalize(path.join(up, e.name)) === path.normalize(base)) continue
      let hit = false
      for (const r of rel) {
        if (path.isAbsolute(r) || r.split(/[\\/]/)[0] !== e.name) continue
        if (await exists(path.join(up, r))) { hit = true; break }
      }
      if (hit) out.siblings.push(e.name)
    }
  }
  return out
}
...
export async function resolveWorktree({ workspace, files }) {
  ...
  /** Windows 路径比较：大小写不敏感（git 报的盘符/目录大小写不一定与传进去的一致）。 */
  ...
  /** 这些路径按这个解析函数找，**真的存在**吗（词法装得下 ≠ 文件在这儿）。 */
  const allExist = async (absOf) => {
    for (const f of declared) if (!(await exists(absOf(f)))) return false
    return true
  }
  const baseIsRepo = await isWorktree(base)
  // ① 会话工作区自己就是仓库、**装得下、文件也在** → 原样返回（正常情形，一个字节都不改）
  //
  // 「文件也在」是 2026-09-20 真机 #3598/#3599 补上的：原来只做**词法**嵌套判断，
  // 而 ulysses/dsh-ulysses-mcp/src/x.py 在 ulysses 仓里**词法上装得下**、文件却根本不在那儿
  // ⇒ 整份声明被拿去 ulysses 里核验 ⇒ ref 查不到（判「ref 无效」）、文件"无改动痕迹"
  // （判「与事实不符」）—— 一份诚实的声明连吃两次假红，两次都是**假声明那一档的措辞**。
  if (baseIsRepo && contains(base) && (await allExist(abs))) {
    return { workspace: base, files: declared, fallback: false, reason: 'session-workspace-is-worktree' }
  }

  // ② 声明写成「**兄弟仓**名 + 仓内路径」（同一个真机形状：会话 cwd 在 ulysses 仓，
  //    产出在隔壁 …\workspace\dsh-ulysses-mcp，声明写成 dsh-ulysses-mcp/src/x.py）。
  //    只多试一个根：base 的**父目录**（= 声明写成父目录相对、少写了一级 ..\ ）。
  //    采用它的条件全部可核对，一条都不靠猜：
  //      · 文件按父目录解析**真的存在**；
  //      · 声明路径的第一段**本身就是那个兄弟仓库的目录名**（拦住"同名文件碰巧在别处"）；
  //      · 全部声明文件**共同落在一个仓库根**上（跨仓库仍然不拼结论）。
  const parentRoot = path.dirname(path.normalize(base))
  if (parentRoot !== path.normalize(base)) {
    const pAbs = (f) => path.resolve(parentRoot, toRelative(parentRoot, f))
    if (await allExist(pAbs)) {
      const head = (f) => toRelative(base, f).replace(/\\/g, '/').split('/')[0]
      const first = head(declared[0])
      const sameHead = first !== '' && first !== '.' && first !== '..'
        && declared.every((f) => head(f) === first)
      const sibRoots = []
      for (const f of declared) {
        const found = await worktreeRoot(path.dirname(pAbs(f)))
        if (found !== null && !sibRoots.includes(found)) sibRoots.push(found)
      }
      const only = sibRoots.length === 1 ? sibRoots[0] : null
      if (sameHead && only !== null && !samePath(only, base) && samePath(only, path.join(parentRoot, first))) {
        const rels = declared.map((f) => path.relative(only, pAbs(f)))
        if (rels.every((r) => r !== '' && !r.startsWith('..') && !path.isAbsolute(r))) {
          return {
            workspace: only,
            files: rels.map((r) => r.replace(/\\/g, '/')),
            fallback: true,
            reason: 'declared-files-in-sibling-worktree',
          }
        }
      }
    }
  ...
  for (const root of roots) {
    ...
    // 回溯出来的就是会话工作区自己 —— 那不算「事实取自别的仓库」。不放行的话结论里会写出
    // "声明的文件不在会话工作区那个仓库里"这种与事实相反的话（文件恰恰就在那儿，只是不在声明的路径上）。
    if (samePath(root, base)) continue
    return {
      workspace: root,
  ...
  // ④ 会话工作区自己是仓库、词法上也装得下，但**声明的文件不在这儿**（真机 #3598/#3599 的假红）：
  //    仍然用它核验（**不猜别的仓库** —— 猜错仓库会把"对质事实"变成"看着像就算"），
  //    但理由必须自报家门：判定器据此把话说成「这些路径不在本仓库」，
  //    而不是「文件在、但你没改」——后者是**假声明**那一档的措辞。
  if (baseIsRepo && contains(base)) {
    return {
      workspace: base,
      files: declared,
      fallback: false,
      reason: 'declared-files-absent-from-session-workspace',
      nearMiss: await subrepoCandidates(base, declared),
    }
  }
  // 找不到仓库时**顺手看一眼子目录与上一级目录**：真机最常见的两种错法是「声明写成了仓库相对路径」
  // （会话 cwd 是几个仓库的父目录，声明 `BLUEPRINT.md` 而不是 `dsh-chatroom/BLUEPRINT.md`）
  // 与「声明写成了父目录相对」（少写一级 ..\）。只用来写提示 —— **绝不自动采用**：
  // 猜错仓库就把「对质事实」变成「看着像就算」。
...
export async function verifyDeclaration({
  ...
  // 「这些路径**根本不在这个仓库里**」是独立一档（真机 2026-09-20 #3598/#3599）：
  // 在那之前它被算进「自上次记录以来无改动痕迹」——同一句话被读成"你没改"，
  // 而事实是"这里没有这个路径"（多半是**声明写的是另一个仓库**）⇒ 一份诚实的声明被标成撒谎档。
  // 判「在」只认 git 与文件系统给出的事实：磁盘上有 / 有未提交改动 / 在这个仓库里有历史 / ref 覆盖它。
  let present = 0
  for (const entry of perFile) {
    // 「磁盘上有这个同名的东西」**不算**「这个仓库知道这个路径」：声明写成 ../ 或绝对路径时，
    // 那个文件可能好端端地在**仓库外面**（真机 #1019 的形状）——那就还是"这里没有它"。
    const absEntry = path.resolve(workspace, entry.path)
    const relEntry = path.relative(workspace, absEntry)
    const inside = relEntry !== '' && !relEntry.startsWith('..') && !path.isAbsolute(relEntry)
    entry.present = (inside && (await exists(absEntry)))
      || entry.dirty || entry.committedAt > 0 || entry.refCovers
    if (entry.present) present++
    // 「**git 结构上看不见这条路径**」：磁盘上有它，但 status 里没有、这个仓库里也没有它的历史
    // ⇒ 它被 .gitignore 排除（或等价地被排除：exclude 文件、被忽略的目录整棵）。
    // 对这样的路径，"没有改动痕迹"**是没有信息量的** —— git 根本给不出痕迹，
    // 拿它去判「与事实不符」等于用"看不见"当"没发生"（真机 2026-09-25 #4831/#4832：data/** 那批）。
    entry.gitInvisible = entry.present === true && entry.dirty !== true
      && entry.committedAt === 0 && entry.refCovers !== true
  }

  // **ref 覆盖之外的额外文件**（837e0518 在 #3863 提的那一档）：这个 commit 到底改了哪些文件，
  // 上面那句 `git show --name-only <ref>` 已经取到手了 —— 但此前只用它给**声明的**文件打 refCovers，
  // **差值那一半被丢掉** ⇒「一条 docs 提交里夹着别人的代码 hunk」在房间里**看不见**，
  // 只有人去读 --stat 才发现（真机 75815b9：message 写的是 docs，实际带着 ulysses/web/app.py +25/-3；
  // 而当事人**没有自曝**，是别人事后用 `git log -S` 追出来的）。
  // 它**不是判据、不改判词**：只进结论当**范围提示** —— 这条提示的净收益就是"不必指望当事人自己想起来"。
  const declaredSet = new Set(perFile.map((f) => norm(f.path)))
  const refExtra = refFiles === null ? [] : [...refFiles].filter((p) => !declaredSet.has(p)).sort()
  ...
  // 一份声明里"没有证据"的那些文件，若**全部**是 git 看不见的，那就不是"不符"，是"查不到"。
  const allInvisible = (list) => list.length > 0 && list.every((f) => f.gitInvisible === true)

  let verdict = VERIFIED
  let reason = 'all-declared-files-have-evidence'
  if (matched.length === 0 && present === 0) {
    // 仓库里没有这些路径的任何痕迹 —— 这不是"没改动"，是"这里没有它"。
    // 排在 ref 那一档前面：ref 查不到往往正是**同一个根因**（查错了仓库）的症状。
    verdict = UNVERIFIED
    reason = 'declared-files-not-in-repo'
  } else if (refState === 'not-found') {
    ...
    } else if (allInvisible(unmatched)) {
      // 声明里的路径 git 一条都看不见（.gitignore 排除的运行时值就是这形状）。
      // 与"没有锚点"同一个道理：**判不出来就老实说判不出来**，不许升级成撒谎档。
      verdict = UNVERIFIED
      reason = 'declared-files-invisible-to-git'
    } else {
      verdict = CONTRADICTED
    ...
    if (anchor === null) {
      ...
      // 一半文件有证据、另一半 git 看不见：看不见的那些并不构成"不符"，整份降为未证实。
      verdict = UNVERIFIED
      reason = 'declared-files-invisible-to-git'
    } else {
      verdict = CONTRADICTED
  ...
  return {
    ...
    refState,
    refTip,
    // ref 覆盖之外的额外文件（范围提示，不是判据；见上面那段说明）
    refExtra,
  }
}
...
function nearMissNote(result) {
  ...
  const sibs = Array.isArray(nm.siblings) ? nm.siblings : []
  ...
  // 「声明写成父目录相对」这一族（真机 #3598/#3599）：上一级目录下面的兄弟仓里有这些路径。
  if (sibs.length > 0) {
    const at = missing.length > 0 ? '，' + missing.join('、') + ' 不存在' : ''
    return '（路径提示：按会话工作目录 ' + base + ' 解析' + at + '；但这些路径在**上一级目录** '
      + path.dirname(base) + ' 下面的 ' + sibs.join('、') + ' 里 —— 声明若是把上一级当基准写的（少写了一级），'
      + '提示到此为止，核验没有替你改路径。）'
  }
...
/**
 * 「这个 commit 还改了哪些**没声明**的文件」——**范围提示**，不是判词（837e0518 #3863）。
 *
 * 为什么要有它：一条 docs 提交里夹着别人的代码 hunk，这种事**只能靠当事人自曝**，
 * 而真机上两次里有一次没人自曝（75815b9，是别人用 `git log -S` 追出来的），
 * 记录行至今写着"改 docstring"。数据本来就在手里（`git show --name-only <ref>`），
 * 丢掉它等于把"看得见"变成"看运气"。它**不改判词**：已证实仍然已证实。
 */
function extraFilesNote(result) {
  const extra = Array.isArray(result.refExtra) ? result.refExtra : []
  if (extra.length === 0) return ''
  return ' ｜ ⚠ **该 commit 还改了 ' + extra.length + ' 个未声明的文件**：'
    + extra.slice(0, 3).join('、') + (extra.length > 3 ? '…（共 ' + extra.length + ' 个）' : '')
    + '（**范围提示，不改判词**）'
}
...
export function describeVerification(result) {
  ...
  const repoNote = result.repoFallback === true && typeof result.workspaceUsed === 'string' && result.workspaceUsed !== ''
    ? '（git 事实取自 ' + path.basename(result.workspaceUsed) + ' 仓库 —— '
      + (result.repoReason === 'declared-files-outside-session-workspace'
        ...
        : result.repoReason === 'declared-files-in-sibling-worktree'
          ? '声明写的是**兄弟仓**里的路径，在那一侧核验'
          : '会话工作区不在 git 里，按声明文件定位') + '）'
    : ''
  if (result.verdict === VERIFIED) {
    return 'git 校验 ✓ 已证实：' + evidenceDetail(result) + extraFilesNote(result) + repoNote
  }
  ...
  // 「这些路径不在这个仓库里」必须与「你没改」分开说（真机 #3598/#3599）：
  // 后者是假声明那一档的措辞，前者只是"这份声明指的可能是另一个仓库"。
  // 「git 看不见这条路径」也要与「你没改」分开说（真机 2026-09-25 #4832：data/** 被判成撒谎档）。
  // 与 #3598/#3599 那一族同形：判据给不出结论时，措辞必须停在"查不到"。
  if (result.reason === 'declared-files-invisible-to-git') {
    const hidden = result.files.filter((f) => !f.evidence).map((f) => f.path).join('、')
    const verifiedPart = result.files.some((f) => f.evidence) ? '（同一份声明里其余文件有 git 证据）' : ''
    return 'git 校验 ? 未证实（git 看不见这些路径：' + hidden + ' —— 被 .gitignore 排除 / 从未入库，'
      + '**不是"没有改动"**：git 对它们结构上给不出痕迹）' + verifiedPart
      + '—— 查不到不等于撒谎，自行判断' + repoNote
  }
  if (result.reason === 'declared-files-not-in-repo') {
    const where = typeof result.workspaceUsed === 'string' && result.workspaceUsed !== ''
      ? path.basename(result.workspaceUsed) + ' 仓库里' : ''
    return 'git 校验 ? 未证实（' + where + '没有这些路径的任何痕迹 —— **不是"没有改动"**，'
      + '多半是声明的路径属于另一个仓库）—— 查不到不等于撒谎，自行判断' + nearMissNote(result)
  }
  const why = {
