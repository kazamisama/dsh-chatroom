 ...
 */
export function assertMessageAppend(state, message) {
  assertStateObject(state)
  requireObj(message, 'message', 'MESSAGE_SHAPE')

  const roomId = message.roomId
  findRoom(state, roomId, 'message.roomId')
  ...
  if (hasSession) {
    ...
    const live = membersOf(state, roomId, true)
    if (!live.some((m) => m.sessionId === sender.sessionId)) {
 ...
 */
export function assertJudgment(state, judgment) {
  assertStateObject(state)
  ...
  const live = membersOf(state, roomId, true)
  if (!live.some((m) => m.sessionId === judgment.sessionId)) {
...
export function assertStateShape(state) {
  ...
  for (const m of messages) {
    try { assertMessageAppend(state, m) } catch (err) { report(err) }
  }
  for (const j of judgments) {
    try { assertJudgment(state, j) } catch (err) { report(err) }
  }
