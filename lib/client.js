...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function noteDiag(kind, ms, kb) {
      var arr = diagSamples[kind]
      arr.push({ ms: ms, at: Date.now(), kb: Number(kb) || 0 })
    ...
    function recentStats(samples, at) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        list.push({ ms: Number(s.ms) || 0, at: s.at, kb: Number(s.kb) || 0 })
      }
      if (list.length === 0) return { count: 0, max: 0, maxAt: 0, median: 0, medianKb: 0 }
    ...
    function createPanel() {
      ...
      if (diagRpc.max >= 500) {
        diagBits.push('拉取 最坏' + Math.round(diagRpc.max) + 'ms' + (diagRpc.maxAt > 0 ? '(' + ago(diagRpc.maxAt) + ')' : '')
          + '/中位' + Math.round(diagRpc.median) + 'ms/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次')
      } else if (diagRpc.count >= 8) {
        diagCalm.push('拉取 中位' + Math.round(diagRpc.median) + 'ms/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次')
      }
    ...
    function refresh(force) {
      ...
      rpc('state').then(function (stateRes) {
        ...
        noteDiag('rpc', performance.now() - diagRpcT0, payloadKb(stateRes))
