...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function createPanel() {
      ...
      // 拉取：最坏 + 中位 + 次数（形状比单个数有用 —— 尖峰与常态要查的地方不一样）
      var diagRpc = recentStats(diagSamples.rpc)
      var diagCand = recentStats(diagSamples.cand)
      if (diagRpc.max >= 500) {
        diagBits.push('拉取 最坏' + Math.round(diagRpc.max) + 'ms' + (diagRpc.maxAt > 0 ? '(' + ago(diagRpc.maxAt) + ')' : '')
          + '/中位' + Math.round(diagRpc.median) + 'ms' + hostBit(diagRpc) + '/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次')
      } else if (diagRpc.count >= 8) {
        diagCalm.push('拉取 中位' + Math.round(diagRpc.median) + 'ms' + hostBit(diagRpc) + '/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次')
      }
      // 候选（抽屉开着时才会轮询）单独报：它和 state 是两个并发 RPC，
      // **"谁在等谁"只有两个数同时有才看得出来**（真机 2026-09-16 的疑问就是这个）
      if (diagCand.count >= 8) {
        diagCalm.push('候选 中位' + Math.round(diagCand.median) + 'ms/' + diagCand.medianKb + 'KB·' + diagCand.count + '次')
      }
      ...
      if (diagCalm.length > 0) {
        head.appendChild(el('span', 'color:' + T.text3 + ';font-size:10px;white-space:nowrap;',
          '· ' + diagCalm.join(' · ')))
      }
