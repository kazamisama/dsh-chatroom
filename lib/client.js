...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
     ...
     */
    var diagSamples = { repaint: [], rpc: [], cand: [] }
    function noteDiag(kind, ms, kb, host) {
      var arr = diagSamples[kind]
      arr.push({ ms: ms, at: Date.now(), kb: Number(kb) || 0, host: Number(host) || 0 })
      if (arr.length > 40) arr.shift()
    }
    /**
     * 主机**自报**的耗时（老宿主没这个字段 → 0，面板据此不显示那一段）。
     *
     * 为什么要它：面板量到的是**往返**，而"慢在通道"与"慢在我"是两个完全不同的处置
     * （前者要改轮询策略，后者要改处理器）。真机 2026-09-16 那个 890ms 就是靠这一减法定性的。
     */
    function hostMsOf(res) {
      try {
        var ms = res !== null && res !== undefined && res.ok === true && res.value ? res.value.ms : null
        return ms !== null && ms !== undefined && typeof ms.total === 'number' ? ms.total : 0
      } catch (e) { return 0 }
    }
    /** 响应载荷多大（KB）。只为诊断量一次字符串化 —— 0.2MB 约 1ms，可接受。 */
    function payloadKb(res) {
      try {
        if (res === null || res === undefined || res.ok !== true) return 0
        return Math.round(JSON.stringify(res.value).length / 1024)
      } catch (e) { return 0 }
    }
    /** 紧凑时长：<1000ms 用 ms，≥1s 用 s（头部只有 380px 宽，2443ms → 2.4s）。 */
    function fmtMs(ms) {
      var v = Math.max(0, Math.round(Number(ms) || 0))
      return v < 1000 ? v + 'ms' : (v / 1000).toFixed(1) + 's'
    }

    /** 主机耗时那一段（老宿主/无样本时不显示，免得写个 0 让人以为"主机不要时间"）。 */
    function hostBit(stats) {
      return stats !== null && stats !== undefined && stats.medianHost > 0
        ? '(主机' + Math.round(stats.medianHost) + 'ms)' : ''
    }
     ...
     */
    /**
     * 一个取样窗口的分布：{ count, max, maxAt, median }（纯函数，可测）。
     *
     * 为什么需要中位（2026-09-16，用户选的方案 a）：面板原来对「拉取」只报**最近 5 分钟最大值**，
     * 而 4457ms 这一个数读不出形状 —— 是"一次尖峰"还是"一直这么慢"？两者要查的方向完全不同
     * （尖峰多半是页面刚加载时那次 candidates 扫描 + 标题解析；常态慢才是这条链路本身）。
     * 一个中位就能分开：最坏 4457/中位 90 = 尖峰；最坏 4457/中位 3000 = 常态。
     * 最大值还要带上**它发生在什么时候** —— "4 分钟前"往往正是页面刚加载那一刻。
     */
    function recentStats(samples, at) {
      ...
      for (var i = 0; i < arr.length; i++) {
        var s = arr[i]
        if (s === null || s === undefined || s.at < cut) continue
        list.push({ ms: Number(s.ms) || 0, at: s.at, kb: Number(s.kb) || 0, host: Number(s.host) || 0 })
      }
      if (list.length === 0) return { count: 0, max: 0, maxAt: 0, median: 0, medianKb: 0, medianHost: 0 }
      var byMs = list.slice().sort(function (a, b) { return a.ms - b.ms })
      var mid = Math.floor(byMs.length / 2)
      var median = byMs.length % 2 === 1 ? byMs[mid].ms : Math.round((byMs[mid - 1].ms + byMs[mid].ms) / 2)
      // 中位载荷（按 ms 排序的那一条的中位位置）—— 用来分"处理慢"与"传得慢"：
      // 528ms/201KB ⇒ 传输/排队；528ms/2KB ⇒ 处理（标题、扫目录）。
      var byKb = list.slice().sort(function (a, b) { return a.kb - b.kb })
      var byHost = list.slice().sort(function (a, b) { return a.host - b.host })
      var worst = byMs[byMs.length - 1]
      return {
        count: byMs.length,
        max: worst.ms,
        maxAt: worst.at,
        median: median,
        medianKb: byKb[mid].kb,
        // 主机自报的中位耗时：往返 − 它 = 通道/排队的代价（老宿主恒 0）
        medianHost: byHost[mid].host,
      }
    }
    ...
    /**
     * 只认「有限、且 ≥0」的数 —— 其余（缺失 / 非数 / NaN / 负数）一律 null。
     *
     * 为什么单独一个函数：面板里凡是"没有就不显示"的读数，都要把这几类坏输入收敛成**同一个**
     * 答案（不显示）。散在各调用点各判一次，迟早有一处漏掉负数或 NaN —— 屏幕上于是出现
     * 「-3ms」或者一个「NaN」，而读数一旦说谎就没人再信它。
     * 只认 number 类型（不收数字字符串）：这是**读数**，宁可不说，也不去猜一个字符串。
     */
    ...
    /**
     * 「这个面板自己花了多少」的一行读数（纯函数，面板测试直接喂）。
     *
     * 两个数分别回答"谁花的"：
     *   room.ms.build —— 宿主侧构建**这一屏**的毫秒数（载荷里已有的字段；老宿主可能没有）；
     *   stats.total / stats.worst —— 本机停摆探针的次数与最坏值，窗口是"自页面加载以来"。
     *
     * 窗口必须说清：头部那行的停摆是「最近 5 分钟」（recentJank 现算），这一行是「自加载」——
     * 两个窗口混着读会得出相反的结论（2026-09-16 那次「9 小时前停了 9 次」就是这么来的）。
     * 所以这里取 jankTotal 与 jankAllWorstMs（同一个窗口的两个数），并在文案里写明「/本页」。
     *
     * 缺失 / 非数 / 负数一律**整段不显示**：这一行宁可整条不出现，也不打一个 undefined，
     * 更不打一个假的 0（写 0 等于告诉读的人"它不要时间"，那比没有读数更糟）。
     */
    function usageLineOf(room, stats) {
      var s = stats === null || stats === undefined ? {} : stats
      var bits = []
      var build = room === null || room === undefined || room.ms === null || room.ms === undefined
        ? null
        : numOrNull(room.ms.build)
      if (build !== null) bits.push('宿主 ' + fmtMs(build))
      var count = numOrNull(s.total)
      if (count !== null) count = Math.floor(count)
      var worst = numOrNull(s.worst)
      if (count !== null && count > 0) {
        bits.push('停摆 ' + count + ' 次/本页' + (worst !== null && worst > 0 ? '(最坏 ' + fmtMs(worst) + ')' : ''))
      }
      return bits.length === 0 ? '' : '用量 ' + bits.join(' · ')
    }
    ...
    function owedLabel(m) {
      ...
      if (seqs.length === 1) {
        return ' · ' + (m.overdue ? '⚠ 逾时未表态 ' : '欠一次表态 ') + '#' + seqs[0]
      }
      return ' · ' + (m.overdue ? '⚠ ' : '') + '欠 ' + seqs.length + ' 条表态，最老 #' + seqs[0]
        + '（另有更晚未回: ' + refs(seqs.slice(1)) + '）'
    }
    ...
    function directionLineOf(m) {
      ...
      var bound = ''
      // 收录范围（真机 #1714）：观察者/静音席位**没有**"边界缺失"这个洞 —— 它们是"按设计不认领"，
      // 所以不给它们挂"⚠ 未声明边界"。⚠ 但这**不等于**"不参与越界判定"：宿主侧 detectOverreach
      // 根本不看 watch（只跳过 enabled === false 的成员与声明者本人），观察者席位只要给了 paths
      // 就照样被判定。旧文案恰恰在这半句上说错了（2026-09-16 更正），而边界文案是**给人做判断用的**，
      // 说错一句就是让人按错的规则动手。
      if (m && m.watch === 'all') {
        return { text: shown === '' ? '' : '方向：' + shown, bound: '观察者席位（收全量变更；领地按它给的 paths 算）' }
      }
      if (m && m.watch === 'wake') {
        return { text: shown === '' ? '' : '方向：' + shown, bound: '唤醒席（全推变更 + 会叫醒；不必回执）' }
      }
      if (m && m.watch === 'feed') {
        return { text: shown === '' ? '' : '方向：' + shown, bound: '只收不答席（变更全推、不产生义务）' }
      }
      if (m && m.watch === 'none') {
        return { text: shown === '' ? '' : '方向：' + shown, bound: '静音席位（不收变更推送；普通发言里 @ 仍叫得到）' }
      }
      if (paths.length > 0) {
    ...
    /**
     * 待表态的逐行文案：谁 · 欠哪几条（真机 #1348 起；2026-09-25 按人重整）。
     * **每人一行，报他自己最老的那条**，不再分"靶子/旧账"两栏 —— 那两栏的前提
     * （靶子会被重新唤醒）在投递层不成立（理由见 owedLabel 上面那段）。
    ...
    function pendingRows(room) {
      ...
      if (detail === null) {
        ...
        return { rows: flat.map(shortOf) }
      }
      // 每人一条（他自己**最老**的那条），不再分"靶子/旧账"两栏 —— 那两栏的前提（靶子会被重新唤醒）
      // 在投递层不成立，理由写在 owedLabel 上面那段注释里。
      return {
        rows: detail.filter(function (d) {
          return d && d.seqs && d.seqs.length > 0
        }).map(function (d) {
          var seqs = d.seqs
          var name = d.shortId || shortOf(d.sessionId)
          return name + '  #' + seqs[0] + (seqs.length > 1 ? '　另 ' + (seqs.length - 1) + ' 条更晚未回' : '')
        }),
      }
    }

    /**
     * 任务状态的中文标签（宿主侧四个取值；没见过的状态**原样带出**，不吞）。
     * 缺失时按 open 处理：任务表是宿主后加的，载荷可能还没带这个字段，
     * 而「待认领」是它最安全的默认（把不存在的东西说成已完成才是真的错）。
     */
    function taskStatusLabel(status) {
      var s = String(status === undefined || status === null ? '' : status)
      if (s === '' || s === 'open') return '待认领'
      if (s === 'claimed') return '已认领'
      if (s === 'done') return '已完成'
      if (s === 'dropped') return '已搁置'
      return s
    }

    /**
     * 任务板的数据整形（纯函数，面板测试直接喂）。
     *
     * 为什么要单独抽出来：tasks 是宿主**新加**的表 —— 载荷里可能还没有这个字段、
     * 可能是 null、元素还可能缺字段。这几种情况如果散在渲染里各判一次，就等于把
     * 「画到一半抛异常」（整帧空白，最难查的那种坏）留给下一次宿主升级。这里统一退回
     * 「空数组 / 空串」，于是「字段缺失」呈现为「这块不出现」，而不是一次崩溃。
     */
    function taskRowsOf(room) {
      var raw = room === null || room === undefined ? null : room.tasks
      if (!Array.isArray(raw)) return []
      // 数组字段（deps / expectPaths）一并规整：它们同样可能缺失、是 null，或者根本不是数组
      var strs = function (list) {
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        // owner 载荷里已经是短号；仍然过一遍 shortOf —— 它幂等，而宿主哪天改成给全长 id 时，
        // 这里也不会把 UUID 的尾巴当成人看的名（取尾巴那个老坑，见 pendingRows 的注释）。
        var owner = t.owner === null || t.owner === undefined || t.owner === '' ? '' : shortOf(t.owner)
        rows.push({
          id: String(t.id || ''),
          title: String(t.title || '') || '(无标题任务)',
          status: taskStatusLabel(t.status),
          // 显示侧直接给文案：面板里每一处问的都是「归谁」，null 与空串在这里合成同一个答案
          owner: owner === '' ? '未认领' : owner,
          deps: strs(t.deps),
          expectPaths: strs(t.expectPaths),
    ...
    /**
     * 任务行的元信息一行：状态 · 归属 · 依赖 · 预期改动。
     * 依赖与预期改动**为空就整段不写**（「依赖 」后面空着比不写更难读）；
     * 状态与归属永远在 —— 它们是「这条任务归谁、走到哪了」的最小答案。
     */
    function taskLineOf(t) {
      if (t === null || t === undefined) return ''
      var bits = []
      var status = String(t.status || '')
      if (status !== '') bits.push(status)
      var owner = String(t.owner || '')
      bits.push(owner === '' ? '未认领' : owner)
      var deps = Array.isArray(t.deps) ? t.deps : []
      var paths = Array.isArray(t.expectPaths) ? t.expectPaths : []
      if (deps.length > 0) bits.push('依赖 ' + deps.join('、'))
      if (paths.length > 0) bits.push('预期改动 ' + paths.join('、'))
      return bits.join(' · ')
    }

    /**
     * 成员行上的「待确认投递」标记（纯函数，面板测试直接喂）。
     *
     * ⚠ 名字必须与待表态（pendingRows / room.pending）分开：那个 pending 是「还欠一次表态」，
     * 这个数是投递台账 —— 消息投出去了、对面还没确认。两件事在同一屏上同时出现，
     * 混用一个词之后就没人分得清是哪一种「没回」。
     *
     * 0 时返回空串：台账绝大多数时候都是 0，给每条成员行挂一个「0 条待确认」
     * 等于每行多一份噪声，真正压着一堆没确认的那个人反而被淹掉。
     */
    function pendingDeliveryLabel(n) {
      var v = Number(n)
      if (!isFinite(v) || v <= 0) return ''
      return Math.floor(v) + ' 条待确认'
    }
    ...
    var S = {
      ...
      // flex:none —— 拖动把手不能被诊断行挤走（它是这个面板唯一的拖动入口）
      grip: 'flex:none;opacity:.55;font-size:11px;font-weight:400;color:' + T.text2 + ';',
        ...
        parts.push(r.room.id + '#' + r.lastSeq + '#' + r.messages.length + '#' + (r.changes || []).length + '#'
          + (r.members || []).map(function (m) {
            return m.shortId + (m.inRoom ? '1' : '0') + (m.owed ? 'o' : '') + (m.overdue ? '!' : '')
              ...
              // 待确认投递数也要进指纹：某人确认掉一条之后，房间里**没有任何消息变化**，
              // 不进指纹这个数就会一直挂着不动（数字说谎比没有数字更糟）。
              + (Number(m.pending) || 0)
          }).join(','))
        // 任务板同理，而且更常见：认领 / 完成一个任务不产生任何消息。不进指纹的话，
        // 面板会一直显示旧状态，直到别的什么事碰巧改了消息 —— 人只会觉得"板子是坏的"。
        parts.push(taskRowsOf(r).map(function (t) {
          return t.id + '@' + t.title + '@' + t.status + '@' + t.owner + '@' + t.updatedAt
        }).join(','))
      }
    ...
    function createPanel() {
      ...
      // 标题**定宽**（flex:none）：它只有五个字，没有理由为诊断行让位 ——
      // 旧写法给它 flex:1，于是诊断行一长，标题先被压成 0 宽（真机截图里整个标题都不见了）。
      // 剩下宽度给诊断行，由它自己省略号（见下面 diagWrap）。
      head.appendChild(el('span', 'flex:none;white-space:nowrap;', '会话聊天室'))
      // 自诊断：异常进橙色告警；**正常时也给一行暗色读数**（下面 diagCalm）。
      // 真机反馈（2026-09-16，用户）：把"拉取"改成分布之后它"没看到那个字段" ——
      // 因为阈值是 500ms，而当时所有拉取都 < 500ms。结论"没问题"是对的，
      // 但人看不到**任何**读数就没法自己判断，于是正常时也报中位数。
      var diagBits = []
      var diagCalm = []
      var diagRepaint = recentMax('repaint')
      if (diagRepaint >= 100) diagBits.push('重绘 ' + fmtMs(diagRepaint))
      // 拉取：中位当主角、最坏跟在后面（形状比单个数有用 —— 尖峰与常态要查的地方不一样）。
      // **紧凑是功能需求**：这一段以前写成「最坏2443ms(刚刚)/中位890ms(主机12ms)/200KB·12次」，
      // 380px 的头部放不下，于是标题被挤没、自己也截断在半个数上（真机截图）。
      var diagRpc = recentStats(diagSamples.rpc)
      var diagCand = recentStats(diagSamples.cand)
      if (diagRpc.max >= 500) {
        diagBits.push('拉取 ' + fmtMs(diagRpc.median) + hostBit(diagRpc) + '/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次'
          + (diagRpc.max > diagRpc.median * 1.5 ? ' 最坏' + fmtMs(diagRpc.max) : ''))
      } else if (diagRpc.count >= 8) {
        diagCalm.push('拉取 ' + fmtMs(diagRpc.median) + hostBit(diagRpc) + '/' + diagRpc.medianKb + 'KB·' + diagRpc.count + '次')
      }
      // 候选（抽屉开着时才会轮询）单独报：它和 state 是两个并发 RPC，
      // **"谁在等谁"只有两个数同时有才看得出来**（真机 2026-09-16 的疑问就是这个）
      if (diagCand.count >= 8) {
        diagCalm.push('候选 ' + fmtMs(diagCand.median) + '/' + diagCand.medianKb + 'KB·' + diagCand.count + '次')
      }
      ...
      // ⚠ UI：这一段的宽度**不可控**（取决于当前有多少个数要报），所以它必须能自己省略 ——
      // 真机截图：旧写法把标题挤成 0 宽、自己还截断在「中位890」上，右边的拖动/抽屉/关闭也跟着被推出面板。
      // 现在：标题定宽不吃亏、诊断行独占剩余宽度并自己打省略号、**全文进悬停**、按钮 flex:none 永不被挤。
      if (diagBits.length > 0 || diagCalm.length > 0) {
        var diagWrap = el('span', 'flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;'
          + 'white-space:nowrap;font-size:10px;')
        if (diagBits.length > 0) {
          diagWrap.appendChild(el('span', 'color:' + T.warn + ';', '⚠ ' + diagBits.join(' · ')))
        }
        if (diagCalm.length > 0) {
          diagWrap.appendChild(el('span', 'color:' + T.text3 + ';',
            (diagBits.length > 0 ? ' · ' : '· ') + diagCalm.join(' · ')))
        }
        // 悬停给出**完整**细节：被省略号吃掉的那部分、以及"这辈子一共几次停摆"
        // （窗口值回答"现在还在发生吗"，累计值回答"这是新问题还是老问题"，两个问题不一样）。
        diagWrap.title = [
          diagRpc.count > 0 ? '拉取：中位 ' + Math.round(diagRpc.median) + 'ms'
            + (diagRpc.medianHost > 0 ? '（其中主机自报 ' + Math.round(diagRpc.medianHost) + 'ms，其余是通道/排队）' : '')
            + '，最坏 ' + Math.round(diagRpc.max) + 'ms' + (diagRpc.maxAt > 0 ? '（' + ago(diagRpc.maxAt) + '）' : '')
            + '，载荷 ' + diagRpc.medianKb + 'KB，样本 ' + diagRpc.count + ' 次' : '',
          diagCand.count > 0 ? '候选：中位 ' + Math.round(diagCand.median) + 'ms，载荷 ' + diagCand.medianKb + 'KB，样本 ' + diagCand.count + ' 次' : '',
          jank.count > 0 ? '停摆：最近 5 分钟 ' + jank.count + ' 次，最坏 ' + (jank.ms / 1000).toFixed(1) + 's'
            + (jank.mine === true ? '（重建中）' : '（非我）') : '',
          '自本页加载以来共 ' + jankTotal + ' 次停摆，最坏 ' + (jankAllWorstMs / 1000).toFixed(1) + 's',
        ].filter(function (s) { return s !== '' }).join('\n')
        head.appendChild(diagWrap)
      }
    ...
    function openPanel(anchor) {
      ...
      refresh(true) // 第一帧不受"打字让路"约束：空面板比一次重绘更打扰人
    ...
    function refresh(force) {
      ...
      // **但"打开面板后的第一帧"必须画**（force）：这条闸门是"别打扰打字"，不是"别显示"。
      // 真机 2026-09-16 撞到的形状：刷新页面后光标仍在 composer（contenteditable）里，
      // 此时打开面板 ⇒ 每一次 4 秒轮询都在这一行提前返回 ⇒ **面板永远空白**（我盯了 80 秒，一帧都没有）。
      // 更糟的是它同时把诊断也饿死了：没有 RPC ⇒ 没有 rpc 取样 ⇒ 头部连"拉取"都不显示。
      if (force !== true && userIsTyping()) return
      ...
      rpc('state').then(function (stateRes) {
        // 连**载荷大小**一起记：只有时长的话，"处理慢"与"传得慢"分不开
        // （真机 2026-09-16：中位 528ms，而插件侧构建只要 15ms、载荷 0.2MB ⇒ 差在别处）
        noteDiag('rpc', performance.now() - diagRpcT0, payloadKb(stateRes), hostMsOf(stateRes))
      ...
      var diagCandT0 = performance.now()
      rpc('candidates').then(function (candRes) {
        // 候选也要计时：它和 state 是两个并发 RPC，**谁在等谁**只有两个数都有才看得出来
        noteDiag('cand', performance.now() - diagCandT0, payloadKb(candRes))
        if (!document.getElementById(PANEL_ID)) return
    ...
    function buildRoom(panel, st) {
      ...
      // 这个面板自己的用量读数。摆在这里而不是新开一节：它和上面那几个数一样是**元信息**
      // （这一屏是谁花的），而不是一条要人去处理的事 —— 单开一节就等于给它一份它没有的重量。
      // 数据只有两个来源，且都是现成的：载荷里的 room.ms.build、本机停摆探针的累计窗口。
      // 两边都没有（老宿主 + 面板刚刷新）时 usageLineOf 给空串 ⇒ 这一格整段不出现，不打 undefined。
      var usage = usageLineOf(room, { total: jankTotal, worst: jankAllWorstMs })
      if (usage !== '') bar.appendChild(el('span', S.weak, usage))
      ...
      // 任务板（**只读**）：任务表是宿主侧的，面板只负责把它摆出来 —— 认领 / 完成都在房间里做，
      // 这里不提供任何写入口（少一个入口就少一处状态能对不上的地方）。
      // 空的时候**整块不出现**：空区块比没有区块更吵，而且会让「这个房间还没有任务」
      // 与「这块坏了」在屏幕上长得一模一样。
      var taskRows = taskRowsOf(room)
      if (taskRows.length > 0) {
        panel.appendChild(el('div', S.section, '任务板（' + taskRows.length + '）'))
        taskRows.forEach(function (t) {
          var trow = el('div', S.row)
          var tleft = el('div', 'flex:1;min-width:0;')
          tleft.appendChild(el('div', '', t.title))
          tleft.appendChild(el('div', S.weak, taskLineOf(t)))
      ...
      room.members.forEach(function (m) {
        ...
        // 待确认投递：0 时 pendingDeliveryLabel 给空串，这一行一个字都不多 ——
        // 台账的意义是「谁压着一堆没确认」，给每个人都挂一个 0 只会把它淹掉。
        var pendingBit = pendingDeliveryLabel(m.pending)
        // "追"的可见性：面板上也要露出"被自动提醒过几次"——否则机器人是在暗处敲人。
        // （room_status 已经露了；这一面是给用户看的。0 时不显示，照 pendingDeliveryLabel 的同一纪律。）
        var remindedBit = Number(m.reminded) > 0 ? ' · 已自动提醒 ' + Math.floor(Number(m.reminded)) + ' 次' : ''
        // 送不出去的提醒次数：审计席指出的缺口"失败只活在 debug 里"，而**面板正是 owner 会看的那一面**。
        var failBit = Number(m.remindFails) > 0 ? ' · ⚠ 提醒 ' + Math.floor(Number(m.remindFails)) + ' 次送不出去' : ''
        left.appendChild(el('div', S.weak, '[' + (m.roleName ? m.roleName + ' · ' : '') + m.shortId + ']'
          + (pendingBit === '' ? '' : ' · ' + pendingBit)
          + ' 已读 ' + m.lastReadSeq
      ...
      if (pend.rows.length > 0) {
        // 【2026-09-25 更正】这句原来写的是"不会自动重发提醒" —— 逾期提醒上线后它就是**假话**，
        // 而**用户直接看的就是这一栏**（我加提醒时只改了 room_status 的表尾，忘了面板；子会话审查照出来的）。
        panel.appendChild(el('div', S.section,
          '待表态（每人最老那条；逾期的会被插件自动追 —— 冷却 30 分、最多 3 次，feed/none 不追；要立刻催谁用 room_alert）'))
        pend.rows.forEach(function (line) { panel.appendChild(el('div', S.dim, line)) })
      }
    ...
    function swap(panel, old) {
      ...
      // **滚动容器有两个**：内容区（消息流）与**抽屉**（成员列表 —— 房间涨到 12 人后要滚很久）。
      // 旧实现只带内容区那一个，于是"滚到中间看某个成员，下一次 4 秒轮询就跳回顶部"
      // （真机反馈 2026-09-16）。两个容器各存各的，别指望一个 anchor 覆盖两处。
      var anchor = scrollAnchorOf(old._content || old)
      var drawerTop = old._drawer === undefined || old._drawer === null ? 0 : old._drawer.scrollTop
      ...
      // 抽屉是**另一个**滚动容器：不恢复它就会每次重画都回到顶部（它没有"贴底跟随"的语义，
      // 所以直接还原像素位置即可）。挂树后再赋值 —— 挂树前 scrollTop 一律被夹成 0。
      if (panel._drawer !== undefined && panel._drawer !== null) panel._drawer.scrollTop = drawerTop
