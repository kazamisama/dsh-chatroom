  ...
  factory: function (require) {
    ...
    function recentStats(samples, at) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        list.push({ ms: Number(s.ms) || 0, at: s.at })
      }
      if (list.length === 0) return { count: 0, max: 0, maxAt: 0, median: 0 }
      list.sort(function (a, b) { return a.ms - b.ms })
      var mid = Math.floor(list.length / 2)
      var median = list.length % 2 === 1 ? list[mid].ms : Math.round((list[mid - 1].ms + list[mid].ms) / 2)
      var worst = list[list.length - 1]
      return { count: list.length, max: worst.ms, maxAt: worst.at, median: median }
    }
    ...
    function createPanel() {
      ...
      if (diagRpc.max < 500 && diagRpc.count >= 8) {
        diagCalm.push('拉取中位 ' + Math.round(diagRpc.median) + 'ms·' + diagRpc.count + '次')
      }
      if (diagRpc.max >= 500) {
        diagBits.push('拉取 最坏' + Math.round(diagRpc.max) + 'ms' + (diagRpc.maxAt > 0 ? '(' + ago(diagRpc.maxAt) + ')' : '')
          + '/中位' + Math.round(diagRpc.median) + 'ms·' + diagRpc.count + '次')
      }
