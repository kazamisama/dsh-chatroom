...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var jankWorstAt = 0
    var jankCount = 0
    /** 最严重那次停摆时，我是否正在重建面板（true = 可能是我的锅）。 */
    var jankWorstMine = false
    ...
    function startJankProbe() {
      ...
      window.setInterval(function () {
        ...
        if (drift >= 600) {
          jankWorstMs = drift
          jankWorstAt = now
          jankWorstMine = renderInFlight === true // 归因：这一次停摆时，面板是不是正在重建
          jankCount++
          clientLog('主线程停摆 ' + Math.round(drift) + ' ms（最近 5 分钟第 ' + jankCount + ' 次）'
    ...
    /** 最近 5 分钟内的最严重停摆（毫秒）；没有就 0。 */
    function recentJankMs() {
      if (jankWorstAt === 0) return 0
      return Date.now() - jankWorstAt < 5 * 60 * 1000 ? jankWorstMs : 0
    }
    ...
    function createPanel() {
      ...
      var jank = recentJankMs()
      if (jank >= 600) {
        diagBits.push('停摆 ' + (jank / 1000).toFixed(1) + 's×' + jankCount
          + (jankWorstMine === true ? '(重建中)' : '(非我)'))
      }
      if (diagBits.length > 0) {
        head.appendChild(el('span', 'color:' + T.warn + ';font-size:10px;white-space:nowrap;',
          '⚠ ' + diagBits.join(' · ')))
      }
    ...
    function apply(ctx) {
      ...
      // 上面为"正在打字"让路，这里在失焦之后把漏掉的那次刷新补上
      document.addEventListener('focusout', function () {
        window.setTimeout(function () { if (!userIsTyping()) reload() }, 120)
      }, true)
